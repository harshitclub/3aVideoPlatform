import { PrismaClient } from "@prisma/client";
import { Request, Response } from "express";
import jwt from "jsonwebtoken";
import { sendGeneratePasswordMail } from "../utils/sendGeneratePasswordMail";
import bcrypt from "bcrypt";

const prisma = new PrismaClient();
const saltRounds = 10;

const expiresIn = 60 * 60 * 24 * 7;

const generateToken = ({ id, empID }: { id: string; empID: string }) => {
  return jwt.sign(
    {
      id: id,
      empID: empID,
    },
    // @ts-ignore
    process.env.JWT_SECRET,
    {
      expiresIn: process.env.JWT_EXPIRE,
    }
  );
};

const generateAccessToken = ({ id, empID }: { id: string; empID: string }) => {
  return jwt.sign(
    {
      id: id,
      empID: empID,
    },
    // @ts-ignore
    process.env.ACCESS_TOKEN_SECRET,
    {
      expiresIn: expiresIn,
    }
  );
};

export const bulkUserSignup = async (req: Request, res: Response) => {
  //   const users = req.body; // This should be an array of user objects

  try {
    const result = await prisma.user.createMany({
      data: [
        {
          empID: "BL0021",
          employeeName: "ARUN KUMAR PANDEY",
          designation: "MANAGER",
          number: "9654790228",
          email: "arunpandeymba@gmail.com ",
          location: "ANAND LIFESPACE DEVELOPMENT LLP",
        },
        {
          empID: "BL0030",
          employeeName: "NEERAJ ",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "7017877302",
          email: "thisisneeraj21@gmail.com ",
          location: "ANAND LIFESPACE DEVELOPMENT LLP",
        },
        {
          empID: "BL0033",
          employeeName: "RAM KISHOR ROY",
          designation: "SENIOR STORE KEEPER",
          number: "7631848430",
          email: "ramkishorroy1982@gmail.com",
          location: "ANAND LIFESPACE DEVELOPMENT LLP",
        },
        {
          empID: "BL0052",
          employeeName: "OM NARAYAN",
          designation: "ASSISTANT FOREMAN",
          number: "8750719041",
          email: "omnarayan2594@gmail.com",
          location: "ANAND LIFESPACE DEVELOPMENT LLP",
        },
        {
          empID: "CL0001",
          employeeName: "ABHISHEK JAIN",
          designation: "ASSISTANT MANAGER",
          number: "9560043041",
          email: "meabhishek.jain@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0008",
          employeeName: "AVANISH KUMAR SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "8808542068",
          email: "avnishsingh312@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0009",
          employeeName: "PARKASH VEER",
          designation: "DEPUTY MANAGER",
          number: "9728234445",
          email: "veer.parkash1@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0010",
          employeeName: "RAJESH KUMAR THAKUR",
          designation: "STORE KEEPER",
          number: "9910640048",
          email: "rkthakur40@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0014",
          employeeName: "PARTOSH PATHAK",
          designation: "FOREMAN",
          number: "9711008630",
          email: "partoshpathak@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0015",
          employeeName: "PRADEEP KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "7037637778",
          email: "pk494022@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0017",
          employeeName: "MOHIT KUMAR",
          designation: "SURVEYOR",
          number: "9718081012",
          email: "mohit91survey@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0019",
          employeeName: "INDRAJEET VISHVAS",
          designation: "FOREMAN",
          number: "9958201445",
          email: "inderv1989@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0020",
          employeeName: "MOHAN SINGH",
          designation: "ASSISTANT- FRONT DESK",
          number: "9627425052",
          email: "manishneginegi29@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0021",
          employeeName: "PREETI SINGH",
          designation: "MANAGER",
          number: "9811154700",
          email: "singhpreeti90@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0022",
          employeeName: "L CHRISTOPHER",
          designation: "SENIOR MANAGER",
          number: "9811514474",
          email: "christlouis@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0023",
          employeeName: "RAHUL MANGLA",
          designation: "GENERAL MANAGER",
          number: "9650433554",
          email: "rahulmangla1981@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0024",
          employeeName: "BRAJESH TANWAR",
          designation: "EXECUTIVE (ERP)",
          number: "9758033976",
          email: "brajesht659@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0025",
          employeeName: "GARVIT GUPTA",
          designation: "DEPUTY GENERAL MANAGER",
          number: "9818691777",
          email: "garvit_gupta2000@yahoo.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0026",
          employeeName: "MANHAR SRIVASTAVA",
          designation: "GENERAL MANAGER",
          number: "7290073417",
          email: "mailmanhar@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0027",
          employeeName: "GOVIND",
          designation: "EXECUTIVE",
          number: "8850811718",
          email: "gy1857310@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0028",
          employeeName: "VIKAS KUMAR SHARMA",
          designation: "ASSISTANT PROJECT MANAGER",
          number: "9897380054",
          email: "vikas22061986@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0029",
          employeeName: "SANTOSH CHAUDHARY",
          designation: "FOREMAN",
          number: "9116534738",
          email: "santoshchoudhary1334@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0031",
          employeeName: "ANSHUMAN PRATIK",
          designation: "GENERAL MANAGER",
          number: "8130231451",
          email: "singh.anshuman021@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "CL0032",
          employeeName: "DEEPAK",
          designation: "EXECUTIVE",
          number: "9813170037",
          email: "ammugurjar25498@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "EW028",
          employeeName: "AKSHIT TYAGI",
          designation: "TRAINEE ENGINEER",
          number: "",
          email: "akshit17052000@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW032",
          employeeName: "HARIOM SHARMA",
          designation: "SENIOR STORE KEEPER",
          number: "9458893442",
          email: "sharmag101076@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW037",
          employeeName: "UDAY KUMAR",
          designation: "SENIOR FOREMAN",
          number: "8882379596",
          email: "udaycucek1@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW038",
          employeeName: "RAJNISH KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "8287442007",
          email: "3628562@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW039",
          employeeName: "PAWAN KUMAR SHUKLA",
          designation: "JUNIOR ENGINEER",
          number: "6394184688",
          email: "shuklapk402@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW040",
          employeeName: "SHUSHIL KUMAR",
          designation: "LAB TECHNICIAN",
          number: "9555228262",
          email: "sunil8076kumar@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW041",
          employeeName: "PANKAJ SATI",
          designation: "ASSISTANT ENGINEER",
          number: "8470005647",
          email: "pankaj.sati1995@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW042",
          employeeName: "SHIVENDER KUMAR",
          designation: "SUPERVISOR",
          number: "9690057666",
          email: "shivendrkali@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW043",
          employeeName: "SANDEEP GOSWAMY",
          designation: "SURVEYOR",
          number: "8810239127",
          email: "sandeepgoswamy121@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW045",
          employeeName: "VIRENDRA BHATIA",
          designation: "FOREMAN",
          number: "9971298206",
          email: "virendrabaitha@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW046",
          employeeName: "PRASHANT KUMAR SINGH",
          designation: "JUNIOR ENGINEER",
          number: "6397914636",
          email: "prasantkhejuri@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW047",
          employeeName: "SHEELENDRA PAL SINGH",
          designation: "DRIVER",
          number: "9990600644",
          email: "singhsheelendra1963@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW048",
          employeeName: "SUSHIL KUMAR YADAV",
          designation: "ASSISTANT MANAGER",
          number: "9910601687",
          email: "yadav.sushil96@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW049",
          employeeName: "MANVIR PRASAD TIWARI",
          designation: "MEDICAL ASSISTANT",
          number: "9571044266",
          email: "mptiwari699@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW050",
          employeeName: "RAVINDRA KUMAR SINGH",
          designation: "GENERAL MANAGER",
          number: "9711098915",
          email: "singhravindrakr@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW051",
          employeeName: "TARUN",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8745935139",
          email: "tarunraj351@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW052",
          employeeName: "MUHMMAD SAKIR ANSARI",
          designation: "FOREMAN",
          number: "8588062983",
          email: "shakirmd859@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW053",
          employeeName: "HARIKESH SINGH YADAV",
          designation: "FOREMAN",
          number: "9971293824",
          email: "harikeshsinghyadav10@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW054",
          employeeName: "RAM NARESH KUMAR",
          designation: "SUPERVISOR",
          number: "9999195156",
          email: "anujk063952@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW056",
          employeeName: "ANUJ KUMAR SHARMA",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9456477481",
          email: "anujmudgal311@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW057",
          employeeName: "ANIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8860588724",
          email: "anilkrsharma67@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW058",
          employeeName: "AMJAD KHAN",
          designation: "FOREMAN",
          number: "8860927419",
          email: "amjadararia88@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW059",
          employeeName: "MOHD INTIKHAB",
          designation: "SENIOR SUPERVISOR",
          number: "8586000733",
          email: "sherurajupuriya@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW060",
          employeeName: "SURENDAR PARSAD",
          designation: "FOREMAN",
          number: "8750029397",
          email: "surendraprasad296@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW061",
          employeeName: "SANJAY KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "9805137201",
          email: "kumarsnj352@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW062",
          employeeName: "RAVI",
          designation: "ASSISTANT FOREMAN",
          number: "9310138091",
          email: "ravir113398@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW063",
          employeeName: "SONU PRAJAPATI",
          designation: "MECHANIC ",
          number: "9811697879",
          email: "sonuprajapati@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW064",
          employeeName: "BIPIN KUMAR MISHRA",
          designation: "SENIOR FOREMAN",
          number: "9555769726",
          email: "bipinmishra5615@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW065",
          employeeName: "VIKASH SHARMA",
          designation: "ASSISTANT FOREMAN",
          number: "8130514290",
          email: "vikkysharma0134@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW066",
          employeeName: "BABLU KUMAR",
          designation: "SENIOR FOREMAN",
          number: "9718261320",
          email: "bkumar1920@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW068",
          employeeName: "SHASHI SHARMA",
          designation: "FOREMAN",
          number: "8459660574",
          email: "sharmashasi03@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "FL0003",
          employeeName: "SUBHASH KUMAR",
          designation: "DEPUTY PROJECT MANAGER",
          number: "7011006123",
          email: "211984.kumar@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0005",
          employeeName: "AMIR KHAN",
          designation: "SENIOR EXECUTIVE",
          number: "9711831932",
          email: "amir.khan@atsgreens.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FL0006",
          employeeName: "KAPIL BHATI",
          designation: "OFFICER",
          number: "9643310605",
          email: "kapil.bhati@atsgreens.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FL0007",
          employeeName: "JITENDRA KUMAR SINGH",
          designation: "SENIOR FOREMAN",
          number: "9643005978",
          email: "",
          location: "HEAD OFFICE",
        },
        {
          empID: "FL0008",
          employeeName: "PARITOSH KUMAR SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "8874040173",
          email: "paritoshkumarsingh88@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0011",
          employeeName: "PHULENA DAS",
          designation: "ASSISTANT FOREMAN",
          number: "9990365064",
          email: "phulenakumar@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0012",
          employeeName: "ALOK BISWAS",
          designation: "SUPERVISOR",
          number: "7303027177",
          email: "atsalok8@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0013",
          employeeName: "ANIL KUMAR",
          designation: "FOREMAN",
          number: "9302903414",
          email: "anilkumar44967@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0015",
          employeeName: "JYOTI JWALA JAINENDRAM",
          designation: "ASSISTANT MANAGER",
          number: "7903164387",
          email: "jainnendram@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FL0020",
          employeeName: "VARUN KUMAR",
          designation: "LAB TECHNICIAN",
          number: "7543827539",
          email: "varunqc@yahoo.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0028",
          employeeName: "MOHSIN  ALAM",
          designation: "SENIOR EXECUTIVE",
          number: "7827685775",
          email: "mohsinaslam009@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FL0029",
          employeeName: "KAMLESH  KUMAR SINGH",
          designation: "ASSISTANT MANAGER",
          number: "8130890035",
          email: "kamleshkumarsingh75@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0032",
          employeeName: "NARENDRA  SINGH",
          designation: "PROJECT ENGINEER",
          number: "8512896826",
          email: "narendrasingh8650@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0034",
          employeeName: "MANTUN KUMAR",
          designation: "SENIOR FOREMAN",
          number: "8860469771",
          email: "mantuntantibg5@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0036",
          employeeName: "GOPAL SINGH",
          designation: "ASSISTANT ENGINEER",
          number: "7982503414",
          email: "gopalpundir777@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0038",
          employeeName: "AKRAM",
          designation: "JUNIOR ENGINEER",
          number: "8745955457",
          email: "iakramsaifi18@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0042",
          employeeName: "ANIL KUMAR",
          designation: "FOREMAN",
          number: "8920153702",
          email: "markchauhan@rediffmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0044",
          employeeName: "UMESH CHAND TYAGI",
          designation: "MEDICAL ASSISTANT",
          number: "8750712542",
          email: " vikastyagi550@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0045",
          employeeName: "SUDHIR KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "9650116455",
          email: "sudhir965011645@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0047",
          employeeName: "SUNIL KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "8130171803",
          email: "sksingh813017@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0048",
          employeeName: "ASHISH SRIVASTAV",
          designation: "JUNIOR ENGINEER",
          number: "7065181322",
          email: "srishish04@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0051",
          employeeName: "DALVEER SINGH",
          designation: "FOREMAN",
          number: "9999447761",
          email: "dvr1978.singh@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0056",
          employeeName: "ANURAG SHAHARWAL",
          designation: "TRAINEE ENGINEER",
          number: "9891516530",
          email: "anurag.shaharwal@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0060",
          employeeName: "SAURABH KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "8750611050",
          email: "saurabh.aitkumar@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0063",
          employeeName: "VIJAY TAANTI",
          designation: "ASSISTANT FOREMAN",
          number: "7042711850",
          email: "vijayraj661987@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0067",
          employeeName: "DEEPAK",
          designation: "PROJECT ENGINEER",
          number: "9818505839",
          email: "deepakthakur617@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0068",
          employeeName: "MANOJ KUMAR",
          designation: "STORE KEEPER",
          number: "8130027853",
          email: "manojkumar27121991@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0070",
          employeeName: "GOVIND KUMAR CHOUDHARY",
          designation: "SENIOR EXECUTIVE",
          number: "9873957536",
          email: "govindimds@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0071",
          employeeName: "GIRIJESH KUMAR MISHRA",
          designation: "FOREMAN",
          number: "7217826322",
          email: "gmishra141980@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0073",
          employeeName: "VINOD KUMAR",
          designation: "FOREMAN",
          number: "8475861064",
          email: "kumarvinod.kunal@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0076",
          employeeName: "HARISH CHANDRA YADAV",
          designation: "FOREMAN",
          number: "8810275476",
          email: "harishyadav6840@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0077",
          employeeName: "RAJAN KUMAR",
          designation: "FOREMAN",
          number: "9599072541",
          email: "rajandhiraniyas@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0080",
          employeeName: "PRABHAT SHARMA",
          designation: "EXECUTIVE",
          number: "9719378767",
          email: "prabhatpandit1998@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0081",
          employeeName: "AATESH GOYAL",
          designation: "PROJECT ENGINEER",
          number: "7428109834",
          email: "goyal.aatesh786@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0082",
          employeeName: '"ZEESHAN ',
          designation: " ALI",
          number: '"',
          email: "ASSISTANT ENGINEER",
          location: "9650869220",
        },
        {
          empID: "FL0083",
          employeeName: "MANOJ KUMAR SHARMA",
          designation: "WELDER",
          number: "9990635845",
          email: "manojsharma63584@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0086",
          employeeName: "AKSHAY CHOUDHARY",
          designation: "ASSISTANT ENGINEER",
          number: "9911054089",
          email: "akshaychoudhary1709@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0087",
          employeeName: "SANTOSH KUMAR",
          designation: "SENIOR FOREMAN",
          number: "9958640594",
          email: "santoshg.kumar123@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0088",
          employeeName: "ANKUSH RAO",
          designation: "SENIOR SUPERVISOR",
          number: "9792202821",
          email: "ankushrao9984@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0089",
          employeeName: "ANGAD KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8285059696",
          email: "angadkumar7541@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0090",
          employeeName: "MANDEEP KUMAR ",
          designation: "OPERATOR",
          number: "9910280064",
          email: " Mandeepkumar97746@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0092",
          employeeName: "MOHAMMAD SALIM",
          designation: "ASSISTANT ENGINEER",
          number: "7503294804",
          email: "mohammadsalim4804@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0093",
          employeeName: "NIRBHAY KUMAR RANJAN",
          designation: "FOREMAN",
          number: "9380957963",
          email: "nirbhayranjan01@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0096",
          employeeName: "SANJEEV KUMAR",
          designation: "ASSISTANT PROJECT MANAGER",
          number: "9871331904",
          email: "sanjeevk406@yahoo.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0098",
          employeeName: "ANUJ KUMAR",
          designation: "SURVEYOR",
          number: "8744036201",
          email: "ak906833@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0099",
          employeeName: "ANKIT SINGH",
          designation: "ASSISTANT",
          number: "6386082928",
          email: "princeksingh699@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0100",
          employeeName: "GIRJESH KUMAR",
          designation: "FOREMAN",
          number: "9667226626",
          email: "girjesh02081987@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0101",
          employeeName: "ALOK KUMAR SHAHI",
          designation: "FOREMAN",
          number: "9599174291",
          email: "alok.kumar930@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0102",
          employeeName: "KRISHNA KUMAR",
          designation: "FOREMAN",
          number: "9540849371",
          email: "krishna.mama06754@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0103",
          employeeName: "RAJU KUMAR",
          designation: "FOREMAN",
          number: "7838828709",
          email: "rajuk056@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0105",
          employeeName: "RAMANAND SAH",
          designation: "FOREMAN",
          number: "8383984767",
          email: "ramanandsahramanandsah46@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0106",
          employeeName: "LAL BAHADUR",
          designation: "ASSISTANT FOREMAN",
          number: "9582988011",
          email: "lalbahadur19909818@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0107",
          employeeName: "MD BABAR KHAN",
          designation: "FOREMAN",
          number: "8287065743",
          email: "babar0626@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0108",
          employeeName: "PREM CHANDRA",
          designation: "SENIOR SUPERVISOR",
          number: "9910849957",
          email: "songra.india@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0109",
          employeeName: "KISHOR KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "7534945322",
          email: "pundhirkishor@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0110",
          employeeName: "AMIT KUMAR SINGH",
          designation: "FOREMAN",
          number: "9318446757",
          email: "amitgsingh@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0111",
          employeeName: "SHRAYANSH",
          designation: "JUNIOR ENGINEER",
          number: "8393859528",
          email: "shrayanshkumar01@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0114",
          employeeName: "SAVIR KHAN",
          designation: "ASSISTANT ENGINEER",
          number: "9599626511",
          email: "savirkhan29011994@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0115",
          employeeName: "AJAY KUMAR SINGH",
          designation: "CONCRETE PUMP OPERATOR",
          number: "7973653727",
          email: "ajaykumara7700@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0116",
          employeeName: "ARJUN SOAM",
          designation: "ASSISTANT ENGINEER",
          number: "8868871979",
          email: "arjunsoam98@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0117",
          employeeName: "ABHIRANJAN CHOUDHARY",
          designation: "SENIOR SUPERVISOR",
          number: "8826980597",
          email: "choudharyabhiranjan@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0118",
          employeeName: "DEEPAK SINGH",
          designation: "FOREMAN",
          number: "8851592221",
          email: "deepakchouhandc5738439@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0119",
          employeeName: "SHIV KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8527973704",
          email: "shivkumar85278@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0120",
          employeeName: "NITISH KUMAR",
          designation: "FOREMAN",
          number: "7011907204",
          email: "nitishkumar25282.nk@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0122",
          employeeName: "BOBY CHAUHAN",
          designation: "ASSISTANT ENGINEER",
          number: "9410629896",
          email: "boby777chauhan@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0123",
          employeeName: "ANIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9654218464",
          email: "anilup80@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0124",
          employeeName: "YOGESH PANCHAL",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9412301205",
          email: "yogeshpanchal713@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0125",
          employeeName: "BRIJMOHAN SINGH YADAV",
          designation: "FOREMAN",
          number: "8800513218",
          email: "bmsingh281@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0126",
          employeeName: "MAHESH KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8930716651",
          email: "mk414239@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0127",
          employeeName: "KAPIL KUMAR",
          designation: "SENIOR MANAGER",
          number: "9711992333",
          email: "kapilkumar7094@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0128",
          employeeName: "ABHISHEK MADHESIYA",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9140150365",
          email: "abh.madheshiya95@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0129",
          employeeName: "AMIT KUMAR",
          designation: "JUNIOR STORE KEEPER",
          number: "9675205231",
          email: "9k3175800@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0130",
          employeeName: "LOKESH KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8743974901",
          email: "rajlokeshp2@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FM0001",
          employeeName: "RAHUL KUMAR",
          designation: "ASSISTANT MANAGER",
          number: "8449479036",
          email: "ahulsharma@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0002",
          employeeName: "SHANKAR JI SINGH",
          designation: "ASSISTANT MANAGER",
          number: "7701815180",
          email: "55471249@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0003",
          employeeName: "NAVIN KUMAR SHARMA",
          designation: "EXECUTIVE (HELPDESK)",
          number: "8076186613",
          email: "nks9958731@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0004",
          employeeName: "MD. HASAN IMAM",
          designation: "SECURITY OFFICER",
          number: "9560898268",
          email: "hasanimam021983@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0005",
          employeeName: "ASHISH BHARGAVA",
          designation: "DEPUTY GENERAL MANAGER",
          number: "8826840666",
          email: "ashish_ttj@rediffmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FM0006",
          employeeName: "SUNNY KUMAR",
          designation: "ESTATE MANAGER",
          number: "8860615132",
          email: "er.sunnynaru@yahoo.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0007",
          employeeName: "BAKEEL",
          designation: "SHIFT ENGINEER",
          number: "8076681828",
          email: "bakeel51685@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0008",
          employeeName: "GAURAV KUMAR",
          designation: "SHIFT ENGINEER",
          number: "9761809836",
          email: "gk509990@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0009",
          employeeName: "RITESH SHARMA",
          designation: "ASSISTANT MANAGER-Accounts",
          number: "7503045594",
          email: "ritesh071991@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0010",
          employeeName: "LOKENDRA SINGH",
          designation: "SHIFT ENGINEER",
          number: "7065793502",
          email: "singhlokendra032@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0011",
          employeeName: "MANEESH KUMAR SINGH",
          designation: "OFFICER (FIRE & SAFETY)",
          number: "8826523863",
          email: "maneeshsingh79@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "FM0012",
          employeeName: "GAURAV SINGH",
          designation: "SR. EXECUTIVE- ACCOUNTS",
          number: "9258918941",
          email: "grvsingh1818@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FM0013",
          employeeName: "MANISH VASHISTH",
          designation: "ASSISTANT MANAGER",
          number: "9911843601",
          email: "vashisth.manish1988@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "FM0014",
          employeeName: "VIKASH KUMAR ",
          designation: "SENIOR EXECUTIVE",
          number: "8006107797",
          email: "vikash210694@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "FM0015",
          employeeName: "AWDHESH KUMAR MISHRA",
          designation: "ASSISTANT MANAGER",
          number: "8851407858",
          email: "awdheshm94@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "FM0016",
          employeeName: "MOHIT KUMAR VERMA",
          designation: "SENIOR EXECUTIVE",
          number: "9718203731",
          email: "mohitverma377@gmail.com",
          location: "HAPPY TRAILS",
        },
        {
          empID: "HK0001",
          employeeName: "SHASHANK MAHESHWARI",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9313934364",
          email: "shashank278@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0008",
          employeeName: "JASBIR SINGH",
          designation: "ASSISTANT VICE PRESIDENT",
          number: "9899899310",
          email: "jasbirsingh7010@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0009",
          employeeName: "SURENDRA KUMAR YADAV",
          designation: "ASSISTANT MANAGER",
          number: "9899915199",
          email: "surendrarahulyadav1989@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0012",
          employeeName: "AVDESH KUMAR JHA",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9810093509",
          email: "avdeshjha23@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0015",
          employeeName: "MANOJ KUMAR SHARMA",
          designation: "SENIOR MANAGER",
          number: "9560888001",
          email: "manojsharma6533@yahoo.in",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0016",
          employeeName: "ANKIT GOENKA",
          designation: "SENIOR GENERAL MANAGER",
          number: "8506082477",
          email: "ankit.goenka@homekraft.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0018",
          employeeName: "NEERAJ SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9891804052",
          email: "bishtneeraj970@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0021",
          employeeName: "KUMAR MALAY",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8757216984",
          email: "kumarmalaysrivastav@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0022",
          employeeName: "SARATH KUMAR PANDIYAN",
          designation: "DEPUTY PROJECT MANAGER",
          number: "8929443149",
          email: "sarath.dce@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0024",
          employeeName: "PUNEET TRIPATHI",
          designation: "SENIOR EXECUTIVE",
          number: "9999330288",
          email: "puneettripathi416@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0027",
          employeeName: "VIKAS SHARMA",
          designation: "DEPUTY MANAGER",
          number: "9873726789",
          email: "vikassharma.ms1983@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0028",
          employeeName: "LAKHAN PAL SINGH",
          designation: "SENIOR MANAGER",
          number: "8527394224",
          email: "lpsingh703@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "HK0029",
          employeeName: "HARISH SHISHAUDIYA",
          designation: "SENIOR EXECUTIVE",
          number: "8851176201",
          email: "harishthakur9582@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0030",
          employeeName: "ASHOK SINGH",
          designation: "SENIOR FOREMAN",
          number: "6397538030",
          email: "ashokkumarji879@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0038",
          employeeName: "DAYAL SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9811044860",
          email: "dsingh319@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0043",
          employeeName: "VIRENDAR SINGH",
          designation: "FOREMAN",
          number: "9718020662",
          email: "virendarsingh7446@gmai.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0044",
          employeeName: "GANESH KUMAR CHOUDHARY",
          designation: "ASSISTANT FOREMAN",
          number: "9990820954",
          email: "ganeshchy5@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0046",
          employeeName: "HANS KUMAR TYAGI",
          designation: "SENIOR MANAGER",
          number: "9953803324",
          email: "hans.tyagi@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0047",
          employeeName: "ARUN KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "9355751220",
          email: "arun.kumarbsacet@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0051",
          employeeName: "KAPIL SINGH",
          designation: "SENIOR ARCHITECT",
          number: "9528336333",
          email: "kapil.arcop@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0053",
          employeeName: "UPENDRA PANDEY",
          designation: "ASSISTANT FOREMAN",
          number: "8800580565",
          email: "upendrapandey534@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0054",
          employeeName: "VIKASH KUMAR MANDAL",
          designation: "FOREMAN",
          number: "9971910327",
          email: "kumarvikas0327@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0055",
          employeeName: "HITESH MITTAL",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9958758365",
          email: "camittal.hitesh@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0057",
          employeeName: "DEEPAK KUMAR",
          designation: "MANAGER",
          number: "9410886250",
          email: "dkshukla.net@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0059",
          employeeName: "GANESH SINGH",
          designation: "ASSISTANT MANAGER",
          number: "8882526688",
          email: "ganeshnegi5@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0061",
          employeeName: "BALJEET SINGH",
          designation: "ASSISTANT MANAGER",
          number: "8527665540",
          email: "baljitryan@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0063",
          employeeName: "GOPAL JI DUBEY",
          designation: "SENIOR-STORE KEEPER",
          number: "9540928112",
          email: "gopaljidubey29@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0067",
          employeeName: "VIJAY KUMAR SINGH",
          designation: "FOREMAN",
          number: "9891970977",
          email: "vijaysinghchendel@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0070",
          employeeName: "GANGA PRASAD PANDIT",
          designation: "ASSISTANT FOREMAN",
          number: "8800604214",
          email: "prasadjigang61@yahoo.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0071",
          employeeName: "VIKAS KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "7906759919",
          email: "vikaskumarrajput986@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0074",
          employeeName: "LOKESH",
          designation: "ASSISTANT MANAGER",
          number: "9555798513",
          email: "lokeshK453@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0078",
          employeeName: "RISHANK RASTOGI",
          designation: "ASSISTANT ENGINEER",
          number: "9971986783",
          email: "rishankrastogi1997@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0079",
          employeeName: "DEEPAK GUPTA",
          designation: "PROJECT MANAGER",
          number: "9999722350",
          email: "deepakguptaa90@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0080",
          employeeName: "PINKU SHARMA",
          designation: "STORE KEEPER",
          number: "9760585453",
          email: "sharmapk0085@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0082",
          employeeName: "RAM KHILAVAN KARSH",
          designation: "ASSISTANT FOREMAN",
          number: "9971857837",
          email: "ramkhilavankarsh007@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0086",
          employeeName: "JITENDER KUMAR",
          designation: "ASSISTANT EQUIPMENT MANAGER",
          number: "8860046628",
          email: "jitender.teotia4384@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0095",
          employeeName: "HARNAM SINGH",
          designation: "FOREMAN",
          number: "8376800268",
          email: "harnamsisodya.2014@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0096",
          employeeName: "MAHESH KUMAR SAHNI",
          designation: "ASSISTANT MANAGER",
          number: "8287039496",
          email: "maheshsa26@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0098",
          employeeName: "ANUJ SEN",
          designation: "SENIOR EXECUTIVE",
          number: "9643156743",
          email: "anujsen214@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0099",
          employeeName: "SABHAJEET YADAV",
          designation: "LAB TECHNICIAN",
          number: "9891228251",
          email: "sabhajeetyadav786@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0100",
          employeeName: "VINOD SINGH",
          designation: "TOWER CRANE OPERATOR",
          number: "9711620332",
          email: "vinodsingh04838@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0101",
          employeeName: "OM PRAKASH PANDEY",
          designation: "CONCRETE PUMP OPERATOR",
          number: "8130568531",
          email: "op.pandey0884@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0102",
          employeeName: "SIDDHARTH SINGH",
          designation: "ASSISTANT MANAGER",
          number: "9452174149",
          email: "siddharth.singh341@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0103",
          employeeName: "NARESH KUMAR",
          designation: "SENIOR FOREMAN",
          number: "9899318292",
          email: "kumarnaresh41285@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0105",
          employeeName: "SAURABH RAWAL",
          designation: "SENIOR MANAGER",
          number: "9013000779",
          email: "srbhsingh445@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0112",
          employeeName: "MOHSIN KHAN",
          designation: "SENIOR MANAGER",
          number: "9718004591",
          email: "khanmohsin563@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0113",
          employeeName: "ANIRUDH KUMAR",
          designation: "SENIOR EXECUTIVE",
          number: "8218483156",
          email: "anirudhvats333@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0117",
          employeeName: "VINAY KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "8447367920",
          email: "yadav.vinay9811@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0118",
          employeeName: "BIJAY KRISHNA SARKAR",
          designation: "LAB TECHNICIAN",
          number: "9312621626",
          email: "bijaykrishana1984@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0119",
          employeeName: "BIJAY KUMAR",
          designation: "FOREMAN",
          number: "8826321036",
          email: "bijay.mcpl@yahoo.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0121",
          employeeName: "JITENDRA KUMAR",
          designation: "FOREMAN",
          number: "9911392073",
          email: "jksingh991139@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0124",
          employeeName: "ANIL KUMAR MISHRA",
          designation: "TOWER CRANE OPERATOR",
          number: "8010457794",
          email: "anilmishra7794@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0125",
          employeeName: "RAHUL KUMAR JHA",
          designation: "PUMP OPERATOR",
          number: "9540232874",
          email: "rahulkumar9540232874@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0129",
          employeeName: "PIYUSH KUMAR SRIVASTAVA",
          designation: "SUPERVISOR",
          number: "9716721907",
          email: "krish10aug@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0130",
          employeeName: "SANJAY KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "9675742003",
          email: "skumarnader@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0131",
          employeeName: "DHARAM PAL SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "9891188406",
          email: "singhdharmpal726@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0135",
          employeeName: "KRISHNA KANT MISHRA",
          designation: "FOREMAN",
          number: "7701928762",
          email: "krishnakantmishra103@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0139",
          employeeName: "JAIMEET SINGH",
          designation: "SENIOR ARCHITECT",
          number: "7838518030",
          email: "joy9311@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0145",
          employeeName: "RAKESH KUMAR THAKUR",
          designation: "STORE KEEPER",
          number: "7814096583",
          email: "rakeshkthakur1974@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0146",
          employeeName: "PRABHASH CHANDRA KUMAR",
          designation: "FOREMAN",
          number: "9999147021",
          email: "pckumar405@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0147",
          employeeName: "MANISH KUMAR GUPTA",
          designation: '"EXECUTIVE VICE PRESIDENT',
          number: '"',
          email: "9999597406",
          location: "ar.mgupta@gmail.com",
        },
        {
          empID: "HK0150",
          employeeName: "PORAS",
          designation: "PROJECT ENGINEER",
          number: "9416239924",
          email: "sardana.poras98@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0152",
          employeeName: "MANOJ KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9873747436",
          email: "manojmansh88@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0153",
          employeeName: "SHUBHAM TANDON",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9650525902",
          email: "shubhamtandon37@ymail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0159",
          employeeName: "CHANDAN KUMAR",
          designation: "FOREMAN",
          number: "9675408989",
          email: "chandankumar1214@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0160",
          employeeName: "PRAVESH CHANDRA SHUKLA",
          designation: "FOREMAN",
          number: "7906262524",
          email: "ps074744@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0164",
          employeeName: "DEVENDRA SINGH",
          designation: "MEDICAL ASSISTANT",
          number: "8384031215",
          email: "d515051967@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0167",
          employeeName: "MOHIT ARORA",
          designation: "CEO (AFFORDABLE HOUSING)",
          number: "9773701030",
          email: "mohit@arorafamily.co.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0171",
          employeeName: "SATISH REDHU",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9992644655",
          email: "satishkandela91@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0175",
          employeeName: "SUNEEL KUMAR",
          designation: "FOREMAN",
          number: "8920971426",
          email: "singhsunil39108@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0177",
          employeeName: "NEERAJ SHARMA",
          designation: "ASSISTANT FOREMAN",
          number: "9717436142",
          email: "neerajsharma41187@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0178",
          employeeName: "KAPIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9718431141",
          email: "kapilkmr.kumar0@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0182",
          employeeName: "SAKSHI SEHGAL",
          designation: "EXECUTIVE ASSISTANT",
          number: "9711072797",
          email: "ms.sahni1988@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0186",
          employeeName: "NIKHIL GOLANI",
          designation: "DEPUTY GENERAL MANAGER",
          number: "9993722238",
          email: "nikhilgolani1@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0190",
          employeeName: "MITHLESH GUPTA",
          designation: "ASSISTANT ENGINEER",
          number: "8285270409",
          email: "mgupta.gupta786@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0191",
          employeeName: "ANIL PANDEY",
          designation: "SENIOR FOREMAN",
          number: "7011470775",
          email: "anilpandey4537@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0195",
          employeeName: "RAVINDER KUMAR",
          designation: "FOREMAN",
          number: "9871575002",
          email: "ravinder.omaxe@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0196",
          employeeName: "DHIRENDRA BAHADUR SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "8851912522",
          email: "sonuar131@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0197",
          employeeName: "PRAVIN KUMAR SHARMA",
          designation: "LAB TECHNICIAN",
          number: "8800643442",
          email: "vipinsharma45662@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0198",
          employeeName: "SANTOSH KUMAR SINGH",
          designation: "SUPERVISER",
          number: "9661027238",
          email: "sksingh96610@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0204",
          employeeName: "AJAY SINGH",
          designation: "FOREMAN",
          number: "8076371034",
          email: "ajaychauhan8076@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0208",
          employeeName: "MOHD KASIM",
          designation: "PROJECT ENGINEER",
          number: "9045331648",
          email: "kasimmansoori.1992@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0209",
          employeeName: "DIPIKA PAL",
          designation: "EXECUTIVE",
          number: "9311820075",
          email: "deepika22august@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0210",
          employeeName: "ANUJ KUMAR",
          designation: "FILING ASSISTANT",
          number: "8929270429",
          email: "anujkumar0868@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0211",
          employeeName: "SHIVAM OJHA",
          designation: "Senior Filing Assistant",
          number: "9821720836",
          email: "toshivam720@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0213",
          employeeName: "KULDEEP KUMAR",
          designation: "STORE KEEPER",
          number: "9911748550",
          email: "kuldeepkumar2658@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0215",
          employeeName: "PRADEEP KUMAR",
          designation: "ASSISTANT MANAGER",
          number: "9811854413",
          email: "pradeep_rkt@rediffmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0216",
          employeeName: "PRINCE SINGH",
          designation: "EXECUTIVE",
          number: "9335625036",
          email: "prince.singh@homekraft.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0217",
          employeeName: "YUVRAJ SINGH",
          designation: "GENERAL MANAGER",
          number: "9769972516",
          email: "singhyuvraj2224@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0218",
          employeeName: "YOGESH KUMAR SHARMA",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "8130496464",
          email: "3cayogesh@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0223",
          employeeName: "ANKIT CHOHAN",
          designation: "ASSISTANT ENGINEER",
          number: "8700822568",
          email: "ankit64chauhan@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0228",
          employeeName: "UMESH KUMAR",
          designation: "SURVEYOR",
          number: "7669021242",
          email: "umesh8316@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0229",
          employeeName: "RANJEET KUMAR GUPTA",
          designation: "SENIOR FOREMAN",
          number: "8851403289",
          email: "ranjeetkumargupta25@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0230",
          employeeName: "RAKESH LAL SRIVASTAVA",
          designation: "DEPUTY GENERAL MANAGER",
          number: "9356946108",
          email: "rakesh.shrivastava@homekraft.in",
          location: "NOBILITY",
        },
        {
          empID: "HK0231",
          employeeName: "SOHAN KUMAR",
          designation: "DRIVER",
          number: "7834847989",
          email: "",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0234",
          employeeName: "UMESH PRASAD",
          designation: "FOREMAN",
          number: "9560825136",
          email: " Up675921prasad@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0241",
          employeeName: "JUNG BAHADUR DUBEY",
          designation: "SENIOR FOREMAN",
          number: "9871856099",
          email: "dubeyjb1@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0243",
          employeeName: "RAJESH KUMAR",
          designation: "SENIOR MANAGER",
          number: "7053043208",
          email: "rajesh.kumar@homekraft.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0244",
          employeeName: "RAM GOPAL",
          designation: "ASSISTANT FOREMAN",
          number: "9560539982",
          email: "rangers72423@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0245",
          employeeName: "MOHAMMAD HILAL",
          designation: "PROJECT ENGINEER",
          number: "9717612501",
          email: "mohdhilal61@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0251",
          employeeName: "SHUBHAM MAURYA",
          designation: "SENIOR EXECUTIVE",
          number: "9892923983",
          email: "shubham.maury1914@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0254",
          employeeName: "SANDEEP KUMAR MISHRA",
          designation: "ASSISTANT FOREMAN",
          number: "8750198311",
          email: "sandeepkmishra@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0255",
          employeeName: "AMIT KUMAR DUBEY",
          designation: "OFFICER",
          number: "9205877903",
          email: "safety.nobility@homekraft.in",
          location: "NOBILITY",
        },
        {
          empID: "HK0256",
          employeeName: "MOHIT SHARMA",
          designation: "ASSISTANT ENGINEER",
          number: "9917995460",
          email: "mohithim101@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0257",
          employeeName: "PANKAJ GOSWAMI",
          designation: "ASSISTANT FOREMAN",
          number: "9891342803",
          email: "pankajgoswami910@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0260",
          employeeName: "YASHPAL SINGH",
          designation: "ASSISTANT ENGINEER",
          number: "9758432778",
          email: " Ys233288@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0262",
          employeeName: "GULAB CHANDRA PAL",
          designation: "ASSISTANT FOREMAN",
          number: "9871863363",
          email: " Gulabpal127@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0265",
          employeeName: "ASHRAF ALI",
          designation: "FOREMAN",
          number: "9821732535",
          email: "ashrafalip4@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0269",
          employeeName: "SANDEEP BHARDWAJ",
          designation: "PROJECT ENGINEER",
          number: "8923089258",
          email: "srbhardwaj.25d1993@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0271",
          employeeName: "PRAVEEN KUMAR SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9870559660",
          email: "praveenkumarsingh362@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0273",
          employeeName: "RAVI RANJAN KUMAR",
          designation: "FOREMAN",
          number: "8750403996",
          email: "raviranjanku008@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0278",
          employeeName: "ARVIND KUMAR",
          designation: "FOREMAN",
          number: "9891864355",
          email: "kmra620@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0279",
          employeeName: "ARUN SHARMA",
          designation: "ARCHITECT",
          number: "9654949369",
          email: "arun.rizvi@yahoo.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0280",
          employeeName: "GAURAV MONGIA",
          designation: "MANAGER",
          number: "9817576578",
          email: "gaurav.mongia@homekraft.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0281",
          employeeName: "VIJAY SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "9911412375",
          email: "vijaysinghduds77@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0283",
          employeeName: "SUKHDARSHAN",
          designation: "SENIOR FOREMAN",
          number: "8700003750",
          email: "sudarshan00005@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0285",
          employeeName: "VIKENDRA SINGH CHAUHAN",
          designation: "SENIOR SURVEYOR",
          number: "9557138992",
          email: "Vikendra.SinghChavhan@homekraft.in",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0286",
          employeeName: "DEEPAK KUMAR",
          designation: "FOREMAN",
          number: "9910789109",
          email: "deepaksinghkws@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0290",
          employeeName: "SANJIT KUMAR RAY",
          designation: "FOREMAN",
          number: "8373997505",
          email: "sanjeet.ray201090@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0292",
          employeeName: "UMASHANKAR",
          designation: "LAB TECHNICIAN",
          number: "9453901021",
          email: "us0461817@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0295",
          employeeName: "DESHRAJ YADAV",
          designation: "LAB TECHNICIAN",
          number: "8447865296",
          email: "",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0300",
          employeeName: "JUHU PRAKASH SHRIVASTAVA",
          designation: "ASSISTANT MANAGER",
          number: "9968004719",
          email: "juhu_shrivastava@yahoo.co.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0301",
          employeeName: "AJEET PRATAP SINGH",
          designation: "STORE KEEPER",
          number: "9808626211",
          email: "singhajrocky@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0302",
          employeeName: "SUNIL PANWAR",
          designation: "SENIOR MANAGER",
          number: "8527528052",
          email: "sunilpanwar277@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0304",
          employeeName: "SHRI SHYAM YADAV",
          designation: "PROJECT ENGINEER",
          number: "9654075419",
          email: "srishyamyadav1991@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0307",
          employeeName: "NASIM AHMAD",
          designation: "GENERAL MANAGER",
          number: "7903907354",
          email: "ahmad.nasim118075@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0310",
          employeeName: "KAUSHAL CHOUHAN",
          designation: "FOREMAN",
          number: "9910570143",
          email: "kaushal.unnati80@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0317",
          employeeName: "BALAM SINGH RAWAT",
          designation: "MEDICAL ASSISTANT",
          number: "9971923673",
          email: "ssrawat07031969@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0324",
          employeeName: "RAM SAKAL SHARMA",
          designation: "FOREMAN",
          number: "8448707163",
          email: "ramsakal9amrapali@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0327",
          employeeName: "BEERESH",
          designation: "FOREMAN",
          number: "9582487994",
          email: "beereshkumar503@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0328",
          employeeName: "ALOK KUMAR TIWARI",
          designation: "SENIOR EXECUTIVE",
          number: "9582595587",
          email: "aloktiwari187@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0333",
          employeeName: "ANANT KUMAR SINHA",
          designation: "GENERAL MANAGER",
          number: "9971138215",
          email: "anantksinha@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0336",
          employeeName: "RAMANIHAL",
          designation: "FOREMAN",
          number: "9598612935",
          email: "ram84157@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0337",
          employeeName: "NEERAJ KUMAR SHARMA",
          designation: "FOREMAN",
          number: "9212165305",
          email: "neerajsharma4932@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0341",
          employeeName: "RANA SINGH",
          designation: "ASSISTANT ENGINEER",
          number: "8279713039",
          email: "singhrana233@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0342",
          employeeName: "RAMEEZ RAZA KHAN",
          designation: "SENIOR EXECUTIVE",
          number: "8750088740",
          email: "rameezraza.khan@homekraft.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0344",
          employeeName: "MANMOHAN SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "8650706458",
          email: "mohanats.hk@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0348",
          employeeName: "BABAR MURMU",
          designation: "TOWER CRANE OPERATOR",
          number: "9873766287",
          email: "babarmumru@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0349",
          employeeName: "ROOP LAL ",
          designation: "FOREMAN",
          number: "8527029117",
          email: "roop98282@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0350",
          employeeName: "YOGENDRA SINGH CHAUHAN",
          designation: "BUILDER HOIST OPERATOR",
          number: "7310744622",
          email: "",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0353",
          employeeName: "SUNIL KUMAR SINGH",
          designation: "FOREMAN",
          number: "9971637083",
          email: "sunil7521@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0354",
          employeeName: "RADHARAMAN  JHA",
          designation: "FOREMAN",
          number: "9910716104",
          email: "rj48jha@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0357",
          employeeName: "SOUVIK RAY",
          designation: "ASSISTANT MANAGER",
          number: "8076577659",
          email: "soumik.survivor@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0358",
          employeeName: "PRATEEK JAIN",
          designation: "DEPUTY MANAGER",
          number: "9990052564",
          email: "prateekjain2021@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0362",
          employeeName: "NAVNEET GUPTA",
          designation: "MANAGER",
          number: "9893736113",
          email: "navneet.tech123@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0363",
          employeeName: "NITYANJAI CHOUDHARY",
          designation: "ARCHITECT",
          number: "9958503817",
          email: "archnj16@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0364",
          employeeName: "PARDEEP KUMAR",
          designation: "FOREMAN",
          number: "7456883587",
          email: "pradeepbazad4433@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0365",
          employeeName: "YAMINI JAIN",
          designation: "EXECUTIVE",
          number: "8447182111",
          email: "minijain.yj@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0367",
          employeeName: "AMIT LAL TRIPATHI",
          designation: "PROJECT ENGINEER",
          number: "9990170978",
          email: "amitlaltripathi2@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0368",
          employeeName: "MANISH SINGH",
          designation: "FOREMAN",
          number: "9990170977",
          email: "LEFT",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0371",
          employeeName: "DEEPAK KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "7017556456",
          email: "deepaksengar8735@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0372",
          employeeName: "DINESH KUMAR",
          designation: "DEPUTY PROJECT MANAGER",
          number: "9871667384",
          email: "dkchaurasia11@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0373",
          employeeName: "ASHOK SINGH",
          designation: "FOREMAN",
          number: "9268572027",
          email: "singhashok0000@gmil.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0374",
          employeeName: "ARVIND KUMAR",
          designation: "FOREMAN",
          number: "8178529722",
          email: "arvinddevrajput@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0375",
          employeeName: "DINESH KUMAR TIWARI",
          designation: "SENIOR EXECUTIVE",
          number: "9717282858",
          email: "tiwaridinesh05@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0377",
          employeeName: "AGHA JARI HUSSAIN",
          designation: "ASSISTANT ENGINEER",
          number: "9717896788",
          email: "aghajari9717@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0379",
          employeeName: "MANSOOR AHMAD",
          designation: "FOREMAN",
          number: "9354967794",
          email: "mansoorahmad9315@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0380",
          employeeName: "RAJAN KUMAR RAJBHAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "",
          email: "rajan.rajbhar@homekraft.in",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0381",
          employeeName: "BINOD KUMAR MEHTA",
          designation: "FOREMAN",
          number: "9891763221",
          email: "amir.khan@atsgreens.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0383",
          employeeName: "VIKRAM SINGH",
          designation: "FOREMAN",
          number: "8376949264",
          email: "kapil.bhati@atsgreens.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0385",
          employeeName: "SATISH CHANDRA RAI",
          designation: "FOREMAN",
          number: "9717701676",
          email: "satishchandrarai23@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0390",
          employeeName: "DHIRENDRA KUMAR SINGH",
          designation: "TIME KEEPER",
          number: "9958338243",
          email: "dhirendra.nh58@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0391",
          employeeName: "KAPIL BERIWAL",
          designation: "SENIOR EXECUTIVE",
          number: "9634766844",
          email: "kapilberiwal2@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0392",
          employeeName: "SRI PRAKASH SINGH",
          designation: "FOREMAN",
          number: "9415870335",
          email: "sriprakash1988@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0393",
          employeeName: "DEEPAK",
          designation: "SENIOR SUPERVISOR",
          number: "9205075894",
          email: "deepak003300@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0398",
          employeeName: "ANKIT DEVPUTRA",
          designation: "DEPUTY MANAGER",
          number: "9711817784",
          email: "ankitdevputra041@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0399",
          employeeName: "PANKAJ KUMAR",
          designation: "FOREMAN",
          number: "8448976320",
          email: "kumarpankaj62618@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0401",
          employeeName: "SIDDHARTHA  SARKAR",
          designation: "GENERAL MANAGER",
          number: "9953699118",
          email: "sarkarsafety@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0403",
          employeeName: "BHUPENDRA SINGH",
          designation: "ASSISTANT MANAGER",
          number: "8800955194",
          email: "bhuppi.nit783@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0405",
          employeeName: "SHASHILESH KUMAR OJHA",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9971558974",
          email: "shashilesh.ojha@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0409",
          employeeName: "Kailash Rathi",
          designation: "ASSISTANT VICE PRESIDENT",
          number: "9871097607",
          email: "rathikailash@yahoo.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0411",
          employeeName: "Sagar Chauhan",
          designation: "EXECUTIVE",
          number: "8587827718",
          email: "sagarchauhan7889@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0414",
          employeeName: "Shailesh Kumar Dwivedi",
          designation: "FOREMAN",
          number: "8527218584",
          email: "shaileshdwivediniu@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0418",
          employeeName: "RAJEEV KUMAR SINGH",
          designation: "FOREMAN",
          number: "8527448722",
          email: "rajeev.singh776@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0420",
          employeeName: "KUNAL SINGH",
          designation: "FOREMAN",
          number: "9891790923",
          email: "1512singh.kumar@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0421",
          employeeName: "Premchand Yadav",
          designation: "FOREMAN",
          number: "9599040820",
          email: "premchandyadav028@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0422",
          employeeName: "Saurabh Rai",
          designation: "PROJECT ENGINEER",
          number: "9990502004",
          email: "shoravx@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0423",
          employeeName: "Rajeev Gupta",
          designation: "SENIOR VICE PRESIDENT",
          number: "9810403257",
          email: "meetrajeevgupta@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0424",
          employeeName: "Atul Kumar",
          designation: "PROJECT ENGINEER",
          number: "9690110311",
          email: "atulsagar1989@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0425",
          employeeName: "Vibhav Pandey",
          designation: "ASSISTANT ENGINEER",
          number: "8318680883",
          email: "vibhavpandey93@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0428",
          employeeName: "DHANANJAY SINGH",
          designation: "FOREMAN",
          number: "9278925793",
          email: "dhananjay.ds833@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0429",
          employeeName: "DHYAN SINGH",
          designation: "FOREMAN",
          number: "9971706624",
          email: "singhdhyan03@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0433",
          employeeName: "Tushar Goel",
          designation: "MANAGER",
          number: "8826880850",
          email: "catushargoel16@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0436",
          employeeName: "NAGENDRA SINGH",
          designation: "SENIOR PROJECT ENGINEER",
          number: "6394631524",
          email: "nagendrasingh@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0438",
          employeeName: "PARDEEP KUMAR",
          designation: "DEPUTY MANAGER",
          number: "9899157182",
          email: "p.arch77@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0439",
          employeeName: "GOVINDA KUMAR",
          designation: "EXECUTIVE",
          number: "7277342943",
          email: "govindakumar72773@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0440",
          employeeName: "CHAITNAY ATTRI",
          designation: "PROJECT ENGINEER",
          number: "9084717878",
          email: "chaitnayattri@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0441",
          employeeName: "VIJAY KUMAR ROY",
          designation: "LAB TECHNICIAN",
          number: "9958892468",
          email: "vijayqc1990@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0442",
          employeeName: "NARESH PANDIT",
          designation: "FOREMAN",
          number: "9572386145",
          email: "nareshpandit9572@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0443",
          employeeName: "NIRANJAN EKKA ",
          designation: "SENIOR SUPERVISOR",
          number: "8929140336",
          email: "niranjanekka1987@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0444",
          employeeName: "RAHUL RAJ",
          designation: "SENIOR MANAGER",
          number: "9999756212",
          email: "raj.fashion007@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0447",
          employeeName: "VISHAL KUMAR",
          designation: "PROJECT ENGINEER",
          number: "9891053778",
          email: "ervishalkumar64@gmai.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0448",
          employeeName: "NITIN KUMAR",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9871210643",
          email: "rm.khnitin@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0449",
          employeeName: "NAJEEB AKHTAR",
          designation: "MANAGER",
          number: "9999219385",
          email: "najeeb.akhter1969@yahoo.co.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0451",
          employeeName: "CHANDRASH SHARMA",
          designation: "ASSISTANT FOREMAN",
          number: "8076236007",
          email: "kuldeepkaushik19944@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0452",
          employeeName: "SANJAY KUMAR THAKUR",
          designation: "ASSISTANT FOREMAN",
          number: "9718961112",
          email: "sanjaythakur17290@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0454",
          employeeName: "PRADEEP KUMAR",
          designation: "EXECUTIVE",
          number: "8882354376",
          email: "pradeepraavi1313@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0455",
          employeeName: "MRIDUL PANWAR",
          designation: "ANALYST",
          number: "9958757727",
          email: "mridul.panwar@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0456",
          employeeName: "ABHISHEK KUMAR",
          designation: "JUNIOR OFFICER",
          number: "9015267015",
          email: "abhi98345@gmial.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0459",
          employeeName: "UPASANA SAJWAN",
          designation: "SENIOR EXECUTIVE",
          number: "9818217199",
          email: "upasanarawat18@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0463",
          employeeName: "VIVEK MISHRA",
          designation: "ASSISTANT ENGINEER",
          number: "8529794176",
          email: "vivekmishra.vm2501@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0465",
          employeeName: "VISHAL KUMAR",
          designation: "SENIOR EXECUTIVE",
          number: "7827470929",
          email: "space.cishalsingh@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0466",
          employeeName: "ARVIND SHARMA",
          designation: "SENIOR EXECUTIVE",
          number: "8837548320",
          email: "arvindsharma9050@gnail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0467",
          employeeName: "BHUPENDRA  SHARMA",
          designation: "TIME KEEPER",
          number: "7982748463",
          email: "sharmabhupendra@44gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0470",
          employeeName: "DAYARAM",
          designation: "FILING ASSISTANT",
          number: "8112622465",
          email: "daya88083@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0472",
          employeeName: "VIJAY KUMAR SHARMA",
          designation: "SENIOR MANAGER",
          number: "8527597878",
          email: "vksharma81@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0474",
          employeeName: "RAJESH SHARMA",
          designation: "TIME KEEPER",
          number: "7906755563",
          email: "rrajeshsharma0176@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0477",
          employeeName: "AKANKSHA SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "8787264944",
          email: "kmakanksha16@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0481",
          employeeName: "ARPIT SINGH",
          designation: "PROJECT ENGINEER",
          number: "7906463360",
          email: "asthakur166@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0483",
          employeeName: "SANJIV KUMAR",
          designation: "FOREMAN",
          number: "7678216159",
          email: "sk645566@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0484",
          employeeName: "SRANJANA BANSAL",
          designation: "GENERAL MANAGER",
          number: "7738052500",
          email: "sranjana2000@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0485",
          employeeName: "HARSH DIXIT",
          designation: "DEPUTY MANAGER",
          number: "8510042259",
          email: "harshdixit01@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0486",
          employeeName: "PARAS SHARMA",
          designation: "ASSISTANT MANAGER",
          number: "9999934530",
          email: "parassharma1990@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0487",
          employeeName: "DEEPAK",
          designation: "FOREMAN",
          number: "8853389011",
          email: "deepaktyagi010@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0488",
          employeeName: "SIDHANTH SINGH",
          designation: "JUNIOR ENGINEER",
          number: "6393159800",
          email: "singhsiddhant024@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0489",
          employeeName: "KHALIQUZZAMA",
          designation: "SENIOR SUPERVISOR",
          number: "9798234067",
          email: "khaliquzzam2014@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0490",
          employeeName: "AWADHESH KUMAR",
          designation: "SENIOR-STORE KEEPER",
          number: "8585981432",
          email: "yadavawadhesh01@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0491",
          employeeName: "CHANDER DEV YADAV",
          designation: "SENIOR LAB TECHNICIAN",
          number: "9304220896",
          email: "chanderdevyadav655@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0492",
          employeeName: "ROSHAN LAL",
          designation: "FOREMAN",
          number: "7303460784",
          email: "roshanlal881976@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0493",
          employeeName: "MOHD WASEEM",
          designation: "FOREMAN",
          number: "9811159498",
          email: "waseemkhanfm@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0494",
          employeeName: "VINAY KUMAR SINGH",
          designation: "FOREMAN",
          number: "9211604679",
          email: "vinay.kumar.87002230@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0495",
          employeeName: "SATYA NARAYAN SINGH",
          designation: "ASSISTANT CONSTRUCTION MANAGER",
          number: "7011433918",
          email: "",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0496",
          employeeName: "RAHUL KUMAR",
          designation: "SENIOR EXECUTIVE",
          number: "8130647892",
          email: "rahulbhatilavish@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0497",
          employeeName: "AMIT KUMAR",
          designation: "SENIOR MANAGER",
          number: "9871424860",
          email: "amit_kumar112@rediffmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0498",
          employeeName: "DEEPAK",
          designation: "ASSISTANT MANAGER",
          number: "8010776102",
          email: "deepak2575636@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0499",
          employeeName: "ABHIMANU KUMAR",
          designation: "PROJECT MANAGER",
          number: "8368126842",
          email: "abhimanu.jpsi@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0500",
          employeeName: "CHANDRAKANT SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9457884605",
          email: "chandrakantsingh10695@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0502",
          employeeName: "SANJIV KUMAR",
          designation: "ASSISTANT PROJECT MANAGER",
          number: "9643302401",
          email: "vermasanj1985@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0503",
          employeeName: "SANJAY SHARMA",
          designation: "SENIOR FOREMAN",
          number: "8851214729",
          email: "sanjaysharmacivil84@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0504",
          employeeName: "SUDHIR KUMAR SINGH",
          designation: "PROJECT MANAGER",
          number: "8377002834",
          email: "sk_10779@yahoo.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0505",
          employeeName: "AMIT KUMAR MISHRA",
          designation: "SENIOR SUPERVISOR",
          number: "8874491722",
          email: "amitkumarmishra@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0506",
          employeeName: "SAKSHI ARORA",
          designation: "EXECUTIVE",
          number: "9953616747",
          email: "sakshiarora@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0507",
          employeeName: "SHIV KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8218072036",
          email: "shivmgp@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0508",
          employeeName: "ANIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9992006803",
          email: "anilahlawat009@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0510",
          employeeName: "RAVI KUMAR",
          designation: "FIELD EXECUTIVE",
          number: "9599211814",
          email: "kamalsubham22@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0512",
          employeeName: "IKRA PARVEEN ",
          designation: "ASSISTANT MANAGER",
          number: "7290836744",
          email: "ikrakhan44@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0513",
          employeeName: "RAJAN",
          designation: "FIELD EXECUTIVE",
          number: "7428699441",
          email: "krajan479@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0514",
          employeeName: "GAURAV SHARMA",
          designation: "SENIOR EXECUTIVE",
          number: "9717306939",
          email: "gauravsharma121189@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0515",
          employeeName: "RASHMI SINGH",
          designation: "ASSISTANT MANAGER",
          number: "8929270381",
          email: "rashmi.singh0917@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0517",
          employeeName: "PRATEEK VERMA",
          designation: "DEPUTY MANAGER",
          number: "8929270432",
          email: "prateek4658@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0518",
          employeeName: "SURYA PRAKASH  RAJPUT",
          designation: "SENIOR EXECUTIVE",
          number: "8595704877",
          email: "sprajput1685@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0519",
          employeeName: "SAURABH YADAV",
          designation: "MANAGER",
          number: "8588978053",
          email: "arsaurav9@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0520",
          employeeName: "ANIL GUPTA",
          designation: "MANAGER",
          number: "8750584958",
          email: "anilgupta0509@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0521",
          employeeName: "CHHATTARPAL SINGH",
          designation: "ASSISTANT MANAGER",
          number: "9711777894",
          email: "cp.singh127032@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0522",
          employeeName: "ANKUR TYAGI",
          designation: "ASSISTANT MANAGER",
          number: "8929449647",
          email: "ankurtyagi751@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0523",
          employeeName: "LOVEKUSH",
          designation: "FILING ASSISTANT",
          number: "8860810962",
          email: "lovekushgee@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0524",
          employeeName: "SACHIN SONI",
          designation: "FIELD EXECUTIVE",
          number: "8800331394",
          email: "sachinsoni7856@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0525",
          employeeName: "PRADEEP KUMAR SINGH",
          designation: "SENIOR FOREMAN",
          number: "9891882597",
          email: "pradeepking2050@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0527",
          employeeName: "UMESH KUMAR GUPTA",
          designation: "DEPUTY MANAGER",
          number: "9711160563",
          email: "umesh0308@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0529",
          employeeName: "ARVIND KUMAR KUNTAL",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9560595178",
          email: "arvind_kuntal@yahoo.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0530",
          employeeName: "NARENDRA SHARMA",
          designation: "FOREMAN",
          number: "9871312449",
          email: "narendrasharma0031@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0533",
          employeeName: "ANOOP KUMAR",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "7291999591",
          email: "anoop.b.bhardwaj@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0534",
          employeeName: "PRAMOD KUMAR",
          designation: "SENIOR EXECUTIVE",
          number: "8178223047",
          email: "pramod_ishanc@yahoo.co.in",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0535",
          employeeName: "ABHIJIT SIDHARTHA",
          designation: "SENIOR MANAGER",
          number: "9582325824",
          email: "abhijeet.siddhartha@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0536",
          employeeName: "TARUN CHAUDHARY",
          designation: "SENIOR MANAGER",
          number: "9818416836",
          email: "tarun20002k@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0537",
          employeeName: "ANIL KUMAR PRAJAPATI",
          designation: "ASSISTANT MANAGER",
          number: "7428708031",
          email: "akpanil.2009@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0538",
          employeeName: "SANDEEP KUMAR KATIYAR",
          designation: "ASSISTANT MANAGER",
          number: "9899012170",
          email: "katiyarsandeep488@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0539",
          employeeName: "SONU KUMAR DUBEY",
          designation: "STORE ASSISTANT",
          number: "7217713979",
          email: "sonudubey496@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "HK0542",
          employeeName: "TARUN KUMAR",
          designation: "PROJECT ENGINEER",
          number: "9643674764",
          email: "tarunkumar9548@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0545",
          employeeName: "BHAIRAV DATT",
          designation: "FOREMAN",
          number: "9871487513",
          email: "bharatdevrari@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0547",
          employeeName: "ANSHUMAN SINGH",
          designation: "JUNIOR ENGINEER",
          number: "7607763463",
          email: "anshusinghrajput2018@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0548",
          employeeName: "TANUJ KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "7835981325",
          email: "tkumar281@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0549",
          employeeName: "SONU KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "9675223794",
          email: "sonukumargarhi10191@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0551",
          employeeName: "AQIL KHAN",
          designation: "ASSISTANT MANAGER",
          number: "8429365951",
          email: "khanaquil1992@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0552",
          employeeName: "BALBINDER SINGH",
          designation: "SENIOR FOREMAN",
          number: "9882944352",
          email: "balbindersingh639358@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0553",
          employeeName: "MONIKA KUMARI",
          designation: "DEPUTY MANAGER",
          number: "9891302607",
          email: "monikafiat06@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0554",
          employeeName: "UDAIVIR ANAND",
          designation: "MANAGING DIRECTOR",
          number: "",
          email: "udaivir.anand@atsgreens.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0555",
          employeeName: "PAWAN KUMAR MAHTO",
          designation: "FOREMAN (LIFT)",
          number: "9999597449",
          email: "pawanmahtoats29@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0556",
          employeeName: "RAJESH KUMAR",
          designation: "FOREMAN",
          number: "8826305492",
          email: "",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0557",
          employeeName: "RAJESH TYAGI",
          designation: "PROJECT ENGINEER",
          number: "9799104228",
          email: "rajeshtyagi9799104882@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0558",
          employeeName: "HARENDRA KISHOR",
          designation: "DEPUTY MANAGER",
          number: "7036703008",
          email: "abc.harendra@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0559",
          employeeName: "SHASHIKANT MOHAN TRIPATHI",
          designation: "FIELD OFFICER (SECURITY)",
          number: "8700726996",
          email: "shashikanmohan.1992@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0560",
          employeeName: "ANMOL BHAYANA",
          designation: "SENIOR MANAGER",
          number: "9811289689",
          email: "anmolbhayanda1@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0561",
          employeeName: "MOHD ADIL KAREEM",
          designation: "PROJECT ENGINEER",
          number: "8505970456",
          email: "adilkareem1993@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0562",
          employeeName: "MONIS RAZA HADAREE",
          designation: "JUNIOR ENGINEER",
          number: "8917015110",
          email: "monisraza0005@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0563",
          employeeName: "PANKAJ KUMAR YADAV",
          designation: "SENIOR FOREMAN",
          number: "8744924906",
          email: "pankajyadav15288@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0564",
          employeeName: "SINGESHWAR PANDIT",
          designation: "FOREMAN",
          number: "9718773507",
          email: "panditsingeshwar1979@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0566",
          employeeName: "SADDAM HUSSAIN",
          designation: "PROJECT ENGINEER",
          number: "8057348755",
          email: "sh91329@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0567",
          employeeName: "NITESH BHATIA",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9986569845",
          email: "niteshbhatia28@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0568",
          employeeName: "ALOK KUMAR SHUKLA",
          designation: "SENIOR EXECUTIVE",
          number: "9582982997",
          email: "aloksfd@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0569",
          employeeName: "MOHD ATIF",
          designation: "SENIOR SUPERVISOR",
          number: "7505526269",
          email: "atif05250@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0570",
          employeeName: "ASFAQUE AHAMAD",
          designation: "SUPERVISOR",
          number: "9118424009",
          email: "asfaque8941@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0571",
          employeeName: "ANKIT PANT",
          designation: "ASSISTANT MANAGER",
          number: "9654046500",
          email: "ankitpant26@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0573",
          employeeName: "MOHAMMAD ARIF",
          designation: "PROJECT ENGINEER",
          number: "7276074608",
          email: "mohdarif1499@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0574",
          employeeName: "SANTOSH SINGH KUSHWAHA",
          designation: "SR. PROJECT ENGINEER",
          number: "9560477120",
          email: "skushwaha85@yahoo.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0575",
          employeeName: "ASHISH KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "8396843993",
          email: "kashish70009@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0576",
          employeeName: "PRAVENDRA SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "9627410494",
          email: "pravendrasingh50255025@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0577",
          employeeName: "ANURAG TYAGI",
          designation: "ASSISTANT ENGINEER",
          number: "9758803334",
          email: "anuragtyagi903@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0579",
          employeeName: "BHAWNA SHARMA",
          designation: "ASSISTANT MANAGER",
          number: "9599038688",
          email: "bhawna18sharma@hotmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0581",
          employeeName: "SHAHNAWAJ TALIB",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9911611160",
          email: "er.shah007@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0582",
          employeeName: "RAVI KUMAR",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "8400977501",
          email: "raviaaho@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0583",
          employeeName: "MITHLESH KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "9910836138",
          email: "mk9765885@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0584",
          employeeName: "PREM CHAND",
          designation: "ASSISTANT FOREMAN",
          number: "7037689359",
          email: "prem616chand@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0585",
          employeeName: "PRADUMAN TIWARI",
          designation: "ASSISTANT ENGINEER",
          number: "8770373205",
          email: "ptiwari9955@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0586",
          employeeName: "MUNEEB UR RAHMAN",
          designation: "ASSISTANT MANAGER",
          number: "9719098671",
          email: "muneeb288@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0587",
          employeeName: "SUMIT KUMAR",
          designation: "ASSISTANT FOREMAN",
          number: "8929703132",
          email: "kumarsumit7740@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0588",
          employeeName: "SUMIT KUMAR SINGH",
          designation: "TIME KEEPER",
          number: "9149326278",
          email: "sumitsharma009149@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0590",
          employeeName: "JYOTI",
          designation: "EXECUTIVE",
          number: "8053384190",
          email: "jyotichauhan4141@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0591",
          employeeName: "DISHA SHARMA",
          designation: "EXECUTIVE",
          number: "8291626583",
          email: "disha3754@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0592",
          employeeName: "ARAB ADHIRAJ",
          designation: "EXECUTIVE",
          number: "7011115771",
          email: "arabadhiraj@gmail.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HK0593",
          employeeName: "AMIT KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8171777767",
          email: "er.amitkumar007@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HK0594",
          employeeName: "SATISH KUMAR",
          designation: "DEPUTY PROJECT MANAGER",
          number: "8955115015",
          email: "satishdhiman1985@gmail.com",
          location: "PIOUS ORCHARDS",
        },
        {
          empID: "HT0232",
          employeeName: "RAJINDER SINGH KOLHI",
          designation: "VICE PRESIDENT",
          number: "9643707606",
          email: "kolhiragindersingh@yahoo.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HT0233",
          employeeName: "BRIJESH KUMAR PANDEY",
          designation: "VICE PRESIDENT",
          number: "9811236488",
          email: "brijesh.pandey@atsgreens.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "HT0234",
          employeeName: "SYED JAFAR RAZA",
          designation: "Head Liaisoning",
          number: "9711098902",
          email: "sjraza@atsgreen.com",
          location: "HEAD OFFICE",
        },
        {
          empID: "FL0003",
          employeeName: "SUBHASH KUMAR",
          designation: "DEPUTY PROJECT MANAGER",
          number: "7011006123",
          email: "211984.kumar@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0008",
          employeeName: "PARITOSH KUMAR SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "8874040173",
          email: "paritoshkumarsingh88@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0011",
          employeeName: "PHULENA DAS",
          designation: "ASSISTANT FOREMAN",
          number: "9990365064",
          email: "phulenakumar@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0012",
          employeeName: "ALOK BISWAS",
          designation: "SUPERVISOR",
          number: "7303027177",
          email: "atsalok8@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0013",
          employeeName: "ANIL KUMAR",
          designation: "FOREMAN",
          number: "9302903414",
          email: "anilkumar44967@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0020",
          employeeName: "VARUN KUMAR",
          designation: "LAB TECHNICIAN",
          number: "7543827539",
          email: "varunqc@yahoo.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0029",
          employeeName: "KAMLESH  KUMAR SINGH",
          designation: "ASSISTANT MANAGER",
          number: "8130890035",
          email: "kamleshkumarsingh75@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0032",
          employeeName: "NARENDRA  SINGH",
          designation: "PROJECT ENGINEER",
          number: "8512896826",
          email: "narendrasingh8650@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0034",
          employeeName: "MANTUN KUMAR",
          designation: "SENIOR FOREMAN",
          number: "8860469771",
          email: "mantuntantibg5@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0036",
          employeeName: "GOPAL SINGH",
          designation: "ASSISTANT ENGINEER",
          number: "7982503414",
          email: "gopalpundir777@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0038",
          employeeName: "AKRAM",
          designation: "JUNIOR ENGINEER",
          number: "8745955457",
          email: "iakramsaifi18@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0042",
          employeeName: "ANIL KUMAR",
          designation: "FOREMAN",
          number: "8920153702",
          email: "markchauhan@rediffmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0044",
          employeeName: "UMESH CHAND TYAGI",
          designation: "MEDICAL ASSISTANT",
          number: "8750712542",
          email: " vikastyagi550@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0045",
          employeeName: "SUDHIR KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "9650116455",
          email: "sudhir965011645@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0047",
          employeeName: "SUNIL KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "8130171803",
          email: "sksingh813017@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0048",
          employeeName: "ASHISH SRIVASTAV",
          designation: "JUNIOR ENGINEER",
          number: "7065181322",
          email: "srishish04@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0051",
          employeeName: "DALVEER SINGH",
          designation: "FOREMAN",
          number: "9999447761",
          email: "dvr1978.singh@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0056",
          employeeName: "ANURAG SHAHARWAL",
          designation: "TRAINEE ENGINEER",
          number: "9891516530",
          email: "anurag.shaharwal@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0060",
          employeeName: "SAURABH KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "8750611050",
          email: "saurabh.aitkumar@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0063",
          employeeName: "VIJAY TAANTI",
          designation: "ASSISTANT FOREMAN",
          number: "7042711850",
          email: "vijayraj661987@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0067",
          employeeName: "DEEPAK",
          designation: "PROJECT ENGINEER",
          number: "9818505839",
          email: "deepakthakur617@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0068",
          employeeName: "MANOJ KUMAR",
          designation: "STORE KEEPER",
          number: "8130027853",
          email: "manojkumar27121991@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0070",
          employeeName: "GOVIND KUMAR CHOUDHARY",
          designation: "SENIOR EXECUTIVE",
          number: "9873957536",
          email: "govindimds@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0071",
          employeeName: "GIRIJESH KUMAR MISHRA",
          designation: "FOREMAN",
          number: "7217826322",
          email: "gmishra141980@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0073",
          employeeName: "VINOD KUMAR",
          designation: "FOREMAN",
          number: "8475861064",
          email: "kumarvinod.kunal@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0076",
          employeeName: "HARISH CHANDRA YADAV",
          designation: "FOREMAN",
          number: "8810275476",
          email: "harishyadav6840@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0077",
          employeeName: "RAJAN KUMAR",
          designation: "FOREMAN",
          number: "9599072541",
          email: "rajandhiraniyas@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0080",
          employeeName: "PRABHAT SHARMA",
          designation: "EXECUTIVE",
          number: "9719378767",
          email: "prabhatpandit1998@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0081",
          employeeName: "AATESH GOYAL",
          designation: "PROJECT ENGINEER",
          number: "7428109834",
          email: "goyal.aatesh786@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0082",
          employeeName: "ZEESHAN ALI ",
          designation: "ASSISTANT ENGINEER",
          number: "9650869220",
          email: "zeeshanchoudhary05@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0083",
          employeeName: "MANOJ KUMAR SHARMA",
          designation: "WELDER",
          number: "9990635845",
          email: "manojsharma63584@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0086",
          employeeName: "AKSHAY CHOUDHARY",
          designation: "ASSISTANT ENGINEER",
          number: "9911054089",
          email: "akshaychoudhary1709@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0087",
          employeeName: "SANTOSH KUMAR",
          designation: "SENIOR FOREMAN",
          number: "9958640594",
          email: "santoshg.kumar123@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0088",
          employeeName: "ANKUSH RAO",
          designation: "SENIOR SUPERVISOR",
          number: "9792202821",
          email: "ankushrao9984@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0089",
          employeeName: "ANGAD KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8285059696",
          email: "angadkumar7541@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0090",
          employeeName: "MANDEEP KUMAR ",
          designation: "OPERATOR",
          number: "9910280064",
          email: " Mandeepkumar97746@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0092",
          employeeName: "MOHAMMAD SALIM",
          designation: "ASSISTANT ENGINEER",
          number: "7503294804",
          email: "mohammadsalim4804@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0093",
          employeeName: "NIRBHAY KUMAR RANJAN",
          designation: "FOREMAN",
          number: "9380957963",
          email: "nirbhayranjan01@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0096",
          employeeName: "SANJEEV KUMAR",
          designation: "ASSISTANT PROJECT MANAGER",
          number: "9871331904",
          email: "sanjeevk406@yahoo.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0098",
          employeeName: "ANUJ KUMAR",
          designation: "SURVEYOR",
          number: "8744036201",
          email: "ak906833@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0099",
          employeeName: "ANKIT SINGH",
          designation: "ASSISTANT",
          number: "6386082928",
          email: "princeksingh699@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0100",
          employeeName: "GIRJESH KUMAR",
          designation: "FOREMAN",
          number: "9667226626",
          email: "girjesh02081987@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0101",
          employeeName: "ALOK KUMAR SHAHI",
          designation: "FOREMAN",
          number: "9599174291",
          email: "alok.kumar930@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0102",
          employeeName: "KRISHNA KUMAR",
          designation: "FOREMAN",
          number: "9540849371",
          email: "krishna.mama06754@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0103",
          employeeName: "RAJU KUMAR",
          designation: "FOREMAN",
          number: "7838828709",
          email: "rajuk056@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0105",
          employeeName: "RAMANAND SAH",
          designation: "FOREMAN",
          number: "8383984767",
          email: "ramanandsahramanandsah46@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0106",
          employeeName: "LAL BAHADUR",
          designation: "ASSISTANT FOREMAN",
          number: "9582988011",
          email: "lalbahadur19909818@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0107",
          employeeName: "MD BABAR KHAN",
          designation: "FOREMAN",
          number: "8287065743",
          email: "babar0626@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0108",
          employeeName: "PREM CHANDRA",
          designation: "SENIOR SUPERVISOR",
          number: "9910849957",
          email: "songra.india@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0109",
          employeeName: "KISHOR KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "7534945322",
          email: "pundhirkishor@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0110",
          employeeName: "AMIT KUMAR SINGH",
          designation: "FOREMAN",
          number: "9318446757",
          email: "amitgsingh@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0111",
          employeeName: "SHRAYANSH",
          designation: "JUNIOR ENGINEER",
          number: "8393859528",
          email: "shrayanshkumar01@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0114",
          employeeName: "SAVIR KHAN",
          designation: "ASSISTANT ENGINEER",
          number: "9599626511",
          email: "savirkhan29011994@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0115",
          employeeName: "AJAY KUMAR SINGH",
          designation: "CONCRETE PUMP OPERATOR",
          number: "7973653727",
          email: "ajaykumara7700@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0116",
          employeeName: "ARJUN SOAM",
          designation: "ASSISTANT ENGINEER",
          number: "8868871979",
          email: "arjunsoam98@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0117",
          employeeName: "ABHIRANJAN CHOUDHARY",
          designation: "SENIOR SUPERVISOR",
          number: "8826980597",
          email: "choudharyabhiranjan@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0118",
          employeeName: "DEEPAK SINGH",
          designation: "FOREMAN",
          number: "8851592221",
          email: "deepakchouhandc5738439@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0119",
          employeeName: "SHIV KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8527973704",
          email: "shivkumar85278@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0120",
          employeeName: "NITISH KUMAR",
          designation: "FOREMAN",
          number: "7011907204",
          email: "nitishkumar25282.nk@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0122",
          employeeName: "BOBY CHAUHAN",
          designation: "ASSISTANT ENGINEER",
          number: "9410629896",
          email: "boby777chauhan@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0123",
          employeeName: "ANIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9654218464",
          email: "anilup80@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0124",
          employeeName: "YOGESH PANCHAL",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9412301205",
          email: "yogeshpanchal713@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0125",
          employeeName: "BRIJMOHAN SINGH YADAV",
          designation: "FOREMAN",
          number: "8800513218",
          email: "bmsingh281@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0126",
          employeeName: "MAHESH KUMAR",
          designation: "PROJECT ENGINEER",
          number: "8930716651",
          email: "mk414239@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0128",
          employeeName: "ABHISHEK MADHESIYA",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9140150365",
          email: "abh.madheshiya95@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0129",
          employeeName: "AMIT KUMAR",
          designation: "JUNIOR STORE KEEPER",
          number: "9675205231",
          email: "9k3175800@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "FL0130",
          employeeName: "LOKESH KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8743974901",
          email: "rajlokeshp2@gmail.com",
          location: "FLORAL PATHWAYS",
        },
        {
          empID: "HK0012",
          employeeName: "AVDESH KUMAR JHA",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "9810093509",
          email: "avdeshjha23@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0038",
          employeeName: "DAYAL SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9811044860",
          email: "dsingh319@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0046",
          employeeName: "HANS KUMAR TYAGI",
          designation: "SENIOR MANAGER",
          number: "9953803324",
          email: "hans.tyagi@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0105",
          employeeName: "SAURABH RAWAL",
          designation: "SENIOR MANAGER",
          number: "9013000779",
          email: "srbhsingh445@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "HK0535",
          employeeName: "ABHIJIT SIDHARTHA",
          designation: "SENIOR MANAGER",
          number: "9582325824",
          email: "abhijeet.siddhartha@gmail.com",
          location: "FLORAL PATHWAYS SALES",
        },
        {
          empID: "EW028",
          employeeName: "AKSHIT TYAGI",
          designation: "TRAINEE ENGINEER",
          number: "8766284934",
          email: "akshit17052000@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW032",
          employeeName: "HARI OM SHARMA",
          designation: "SENIOR STORE KEEPER",
          number: "9458893442",
          email: "sharmag101076@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW037",
          employeeName: "UDAY KUMAR",
          designation: "SENIOR FOREMAN",
          number: "8882379596",
          email: "udaycucek1@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW038",
          employeeName: "RAJNISH KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "8287442007",
          email: "rk3628562@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW039",
          employeeName: "PAWAN KUMAR SHUKLA",
          designation: "JUNIOR ENGINEER",
          number: "6394184688",
          email: "shuklapk402@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW040",
          employeeName: "SHUSHIL KUMAR",
          designation: "LAB TECHNICIAN",
          number: "9555228262",
          email: "sunil8076kumar@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW041",
          employeeName: "PANKAJ SATI",
          designation: "ASSISTANT ENGINEER",
          number: "8470005647",
          email: "pankaj.sati1995@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW042",
          employeeName: "SHIVENDER KUMAR",
          designation: "SUPERVISOR",
          number: "9690057666",
          email: "shivendrkali@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW043",
          employeeName: "SANDEEP GOSWAMY",
          designation: "SURVEYOR",
          number: "8810239127",
          email: "sandeepgoswamy121@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW045",
          employeeName: "VIRENDRA BHATIA",
          designation: "FOREMAN",
          number: "9971298206",
          email: "virendrabaitha@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW046",
          employeeName: "PRASHANT KUMAR SINGH",
          designation: "JUNIOR ENGINEER",
          number: "6397914636",
          email: "prashantkhejuri@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW047",
          employeeName: "SHEELENDRA PAL SINGH",
          designation: "DRIVER",
          number: "9990600644",
          email: "singhsheelendra1963@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW048",
          employeeName: "SUSHIL KUMAR YADAV",
          designation: "ASSISTANT MANAGER",
          number: "9910601687",
          email: "yadav.sushil96@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW049",
          employeeName: "MANVIR PRASAD TIWARI",
          designation: "MEDICAL ASSISTANT",
          number: "9571044266",
          email: "mptiwari699@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW050",
          employeeName: "RAVINDRA KUMAR SINGH",
          designation: "GENERAL MANAGER",
          number: "9711098915",
          email: "singhravindrakr@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW051",
          employeeName: "TARUN",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8745935139",
          email: "tarunraj351@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW052",
          employeeName: "MOHAMMAD SHAKIR ANSARI",
          designation: "FOREMAN",
          number: "8588062983",
          email: "shakirmd859@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW053",
          employeeName: "HARIKESH SINGH YADAV",
          designation: "FOREMAN",
          number: "9971293824",
          email: "harikeshsinghyadav501@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW054",
          employeeName: "RAM NARESH KUMAR",
          designation: "SUPERVISOR",
          number: "9999195156",
          email: "anujk063952@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW056",
          employeeName: "ANUJ KUMAR SHARMA",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9456477481",
          email: "anujmudgal311@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW057",
          employeeName: "ANIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8860588724",
          email: "anilkrsharma67@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW058",
          employeeName: "AMJAD KHAN",
          designation: "FOREMAN",
          number: "8860927419",
          email: "amjadararia88@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW059",
          employeeName: "MOHD INTIKHAB",
          designation: "SENIOR SUPERVISOR",
          number: "8586000733",
          email: "sherurajupuriya21@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW060",
          employeeName: "SURENDAR PARSAD",
          designation: "FOREMAN",
          number: "8750029397",
          email: "surendraprasad296@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW061",
          employeeName: "SANJAY KUMAR",
          designation: "ASSISTANT ENGINEER",
          number: "9805137201",
          email: "kumarsnj352@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW062",
          employeeName: "RAVI",
          designation: "ASSISTANT FOREMAN",
          number: "9310138091",
          email: "ravir113398@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW063",
          employeeName: "SONU PRAJAPATI",
          designation: "MECHANIC ",
          number: "9811697879",
          email: "sonuprajapati86836@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "HK0539",
          employeeName: "SONU KUMAR DUBEY",
          designation: "STORE ASSISTANT",
          number: "7217713979",
          email: "sonudubey496@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW064",
          employeeName: "BIPIN KUMAR MISHARA",
          designation: "SR.FM",
          number: "9555769726",
          email: "bipinmishra5615@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW065",
          employeeName: "VIKASH SHARMA",
          designation: "AFM",
          number: "8130514290",
          email: "vikkysharma0134@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "EW066",
          employeeName: "BABLU KUMAR",
          designation: "SR.FM",
          number: "9718261320",
          email: "bkumar1920@gmail.com",
          location: "EWS-Govindpuram",
        },
        {
          empID: "CL0001",
          employeeName: "ABHISHEK JAIN",
          designation: "ASSISTANT MANAGER",
          number: "9560043041",
          email: "meabhishek.jain@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0008",
          employeeName: "AVANISH KUMAR SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "8808542068",
          email: "avnishsingh312@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0009",
          employeeName: "PARKASH VEER",
          designation: "DEPUTY MANAGER",
          number: "9728234445",
          email: "veer.parkash1@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0010",
          employeeName: "RAJESH KUMAR THAKUR",
          designation: "STORE KEEPER",
          number: "9910640048",
          email: "Rkthakur40@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0014",
          employeeName: "PARTOSH PATHAK",
          designation: "FOREMAN",
          number: "9711008630",
          email: "partoshpathak@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0015",
          employeeName: "PRADEEP KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "7037637778",
          email: "pk494022@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0017",
          employeeName: "MOHIT KUMAR",
          designation: "SURVEYOR",
          number: "9718081012",
          email: "mohit91survey@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0019",
          employeeName: "INDRAJEET VISHVAS",
          designation: "FOREMAN",
          number: "9958201445",
          email: "inderjeetvishvash844@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0020",
          employeeName: "MOHAN SINGH",
          designation: "ASSISTANT- FRONT DESK",
          number: "9627425052",
          email: "manishneginegi29@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0027",
          employeeName: "GOVIND",
          designation: "EXECUTIVE",
          number: "7800922198",
          email: "gy1857310@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0028",
          employeeName: "VIKAS KUMAR SHARMA",
          designation: "ASSISTANT PROJECT MANAGER",
          number: "9897380054",
          email: "vikas22061986@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0029",
          employeeName: "SANTOSH CHAUDHARY",
          designation: "FOREMAN",
          number: "9116534738",
          email: "santoshchoudhary1334@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "CL0032",
          employeeName: "DEEPAK",
          designation: "EXECUTIVE",
          number: "9813170037",
          email: "ammugurjar25498@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "HK0028",
          employeeName: "LAKHAN PAL SINGH",
          designation: "SENIOR MANAGER",
          number: "8527394224",
          email: "lpsingh703@gmail.com",
          location: "SOHNA",
        },
        {
          empID: "FM0012",
          employeeName: "GAURAV SINGH",
          designation: "SR. EXECUTIVE- ACCOUNTS",
          number: "9258918941",
          email: "grvsingh1818@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "FM0013",
          employeeName: "MANISH VASHISTH",
          designation: "ASSISTANT MANAGER",
          number: "9911843601",
          email: "vashisth.manish1988@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "FM0014",
          employeeName: "VIKASH KUMAR",
          designation: "SENIOR EXECUTIVE",
          number: "8006107797",
          email: "vikash210694@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "FM0015",
          employeeName: "AWDHESH KUMAR MISHRA",
          designation: "ASSISTANT MANAGER",
          number: "8851407858",
          email: "awdheshm94@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0053",
          employeeName: "UPENDRA PANDEY",
          designation: "ASSISTANT FOREMAN",
          number: "8800580565",
          email: "upendrapandey534@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0063",
          employeeName: "GOPAL JI DUBEY",
          designation: "SENIOR-STORE KEEPER",
          number: "9540928112",
          email: "gopaljidubey29@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0070",
          employeeName: "GANGA PRASAD PANDIT",
          designation: "ASSISTANT FOREMAN",
          number: "8800604214",
          email: "prasadjigang61@yahoo.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0071",
          employeeName: "VIKAS KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "7906759919",
          email: "vikaskumarrajput986@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0074",
          employeeName: "LOKESH",
          designation: "ASSISTANT MANAGER",
          number: "9555798513",
          email: "lokeshK453@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0078",
          employeeName: "RISHANK RASTOGI",
          designation: "ASSISTANT ENGINEER",
          number: "9971986783",
          email: "rishankrastogi1997@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0079",
          employeeName: "DEEPAK GUPTA",
          designation: "PROJECT MANAGER",
          number: "9999722350",
          email: "deepakguptaa90@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0099",
          employeeName: "SABHAJEET YADAV",
          designation: "LAB TECHNICIAN",
          number: "9891228251",
          email: "sabhajeetyadav786@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0101",
          employeeName: "OM PRAKASH PANDEY",
          designation: "CONCRETE PUMP OPERATOR",
          number: "8130568531",
          email: "op.pandey0884@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0118",
          employeeName: "BIJAY KRISHNA SARKAR",
          designation: "LAB TECHNICIAN",
          number: "9312621626",
          email: "bijaykrishana1984@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0121",
          employeeName: "JITENDRA KUMAR",
          designation: "FOREMAN",
          number: "9911392073",
          email: "jksingh991139@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0130",
          employeeName: "SANJAY KUMAR",
          designation: "SENIOR SUPERVISOR",
          number: "9675742003",
          email: "skumarnader@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0131",
          employeeName: "DHARAM PAL SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "9891188406",
          email: "singhdharmpal726@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0152",
          employeeName: "MANOJ KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9873747436",
          email: "manojmansh88@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0159",
          employeeName: "CHANDAN KUMAR",
          designation: "FOREMAN",
          number: "9675408989",
          email: "chandankumar1214@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0160",
          employeeName: "PRAVESH CHANDRA SHUKLA",
          designation: "FOREMAN",
          number: "7906262524",
          email: "ps074744@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0164",
          employeeName: "DEVENDRA SINGH",
          designation: "MEDICAL ASSISTANT",
          number: "8384031215",
          email: "d515051967@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0171",
          employeeName: "SATISH REDHU",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9992644655",
          email: "satishkandela91@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0175",
          employeeName: "SUNEEL KUMAR",
          designation: "FOREMAN",
          number: "8920971426",
          email: "singhsunil39108@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0195",
          employeeName: "RAVINDER KUMAR",
          designation: "FOREMAN",
          number: "9871575002",
          email: "ravinder.omaxe@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0196",
          employeeName: "DHIRENDRA BAHADUR SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "8851912522",
          email: "sonuar131@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0208",
          employeeName: "MOHD KASIM",
          designation: "PROJECT ENGINEER",
          number: "9045331648",
          email: "kasimmansoori.1992@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0213",
          employeeName: "KULDEEP KUMAR",
          designation: "STORE KEEPER",
          number: "9911748550",
          email: "kuldeepkumar2658@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0228",
          employeeName: "UMESH KUMAR",
          designation: "SURVEYOR",
          number: "7669021242",
          email: "umesh8316@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0229",
          employeeName: "RANJEET KUMAR GUPTA",
          designation: "SENIOR FOREMAN",
          number: "8851403289",
          email: "ranjeetkumargupta25@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0230",
          employeeName: "RAKESH LAL SRIVASTAVA",
          designation: "DEPUTY GENERAL MANAGER",
          number: "9356946108",
          email: "rakesh.shrivastava@homekraft.in",
          location: "NOBILITY",
        },
        {
          empID: "HK0241",
          employeeName: "JUNG BAHADUR DUBEY",
          designation: "SENIOR FOREMAN",
          number: "9871856099",
          email: "dubeyjb1@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0255",
          employeeName: "AMIT KUMAR DUBEY",
          designation: "OFFICER",
          number: "9205877903",
          email: "safety.nobility@homekraft.in",
          location: "NOBILITY",
        },
        {
          empID: "HK0256",
          employeeName: "MOHIT SHARMA",
          designation: "ASSISTANT ENGINEER",
          number: "9917995460",
          email: "mohithim101@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0257",
          employeeName: "PANKAJ GOSWAMI",
          designation: "ASSISTANT FOREMAN",
          number: "9891342803",
          email: "pankajgoswami910@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0281",
          employeeName: "VIJAY SINGH",
          designation: "ASSISTANT FOREMAN",
          number: "9911412375",
          email: "vijaysinghduds77@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0283",
          employeeName: "SUKHDARSHAN",
          designation: "SENIOR FOREMAN",
          number: "8700003750",
          email: "sudarshan00005@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0286",
          employeeName: "DEEPAK KUMAR",
          designation: "FOREMAN",
          number: "9910789109",
          email: "deepaksinghkws@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0353",
          employeeName: "SUNIL KUMAR SINGH",
          designation: "FOREMAN",
          number: "9971637083",
          email: "sunil7521@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0354",
          employeeName: "RADHARAMAN  JHA",
          designation: "FOREMAN",
          number: "9910716104",
          email: "rj48jha@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0374",
          employeeName: "ARVIND KUMAR",
          designation: "FOREMAN",
          number: "8178529722",
          email: "arvinddevrajput@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0375",
          employeeName: "DINESH KUMAR TIWARI",
          designation: "SENIOR EXECUTIVE",
          number: "9717282858",
          email: "tiwaridinesh05@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0393",
          employeeName: "DEEPAK",
          designation: "SENIOR SUPERVISOR",
          number: "9205075894",
          email: "deepak003300@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0405",
          employeeName: "SHASHILESH KUMAR OJHA",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9971558974",
          email: "shashilesh.ojha@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0414",
          employeeName: "Shailesh Kumar Dwivedi",
          designation: "FOREMAN",
          number: "8527218584",
          email: "shaileshdwivediniu@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0424",
          employeeName: "Atul Kumar",
          designation: "PROJECT ENGINEER",
          number: "9690110311",
          email: "atulsagar1989@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0425",
          employeeName: "Vibhav Pandey",
          designation: "ASSISTANT ENGINEER",
          number: "8318680883",
          email: "vibhavpandey93@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0429",
          employeeName: "DHYAN SINGH",
          designation: "FOREMAN",
          number: "9971706624",
          email: "singhdhyan03@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0447",
          employeeName: "VISHAL KUMAR",
          designation: "PROJECT ENGINEER",
          number: "9891053778",
          email: "ervishalkumar64@gmai.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0451",
          employeeName: "CHANDRASH SHARMA",
          designation: "ASSISTANT FOREMAN",
          number: "8076236007",
          email: "kuldeepkaushik19944@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0452",
          employeeName: "SANJAY KUMAR THAKUR",
          designation: "ASSISTANT FOREMAN",
          number: "9718961112",
          email: "sanjaythakur17290@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0456",
          employeeName: "ABHISHEK KUMAR",
          designation: "JUNIOR OFFICER",
          number: "9015267015",
          email: "abhi98345@gmial.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0463",
          employeeName: "VIVEK MISHRA",
          designation: "ASSISTANT ENGINEER",
          number: "8529794176",
          email: "vivekmishra.vm2501@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0467",
          employeeName: "BHUPENDRA  SHARMA",
          designation: "TIME KEEPER",
          number: "7982748463",
          email: "sharmabhupendra@44gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0487",
          employeeName: "DEEPAK",
          designation: "FOREMAN",
          number: "8853389011",
          email: "deepaktyagi010@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0489",
          employeeName: "KHALIQUZZAMA",
          designation: "SENIOR SUPERVISOR",
          number: "9798234067",
          email: "khaliquzzam2014@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0499",
          employeeName: "ABHIMANU KUMAR",
          designation: "PROJECT MANAGER",
          number: "8368126842",
          email: "abhimanu.jpsi@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0500",
          employeeName: "CHANDRAKANT SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9457884605",
          email: "chandrakantsingh10695@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0562",
          employeeName: "MONIS RAZA HADAREE",
          designation: "JUNIOR ENGINEER",
          number: "8917015110",
          email: "monisraza0005@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0566",
          employeeName: "SADDAM HUSSAIN",
          designation: "PROJECT ENGINEER",
          number: "8057348755",
          email: "sh91329@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0573",
          employeeName: "MOHAMMAD ARIF",
          designation: "PROJECT ENGINEER",
          number: "7276074608",
          email: "mohdarif1499@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0584",
          employeeName: "PREM CHAND",
          designation: "ASSISTANT FOREMAN",
          number: "7037689359",
          email: "prem616chand@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0587",
          employeeName: "SUMIT KUMAR",
          designation: "ASSISTANT FOREMAN",
          number: "8929703132",
          email: "kumarsumit7740@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0588",
          employeeName: "SUMIT KUMAR SHARMA",
          designation: "TIME KEEPER",
          number: "9149326278",
          email: "sumitsharma009149@gmail.com",
          location: "NOBILITY",
        },
        {
          empID: "HK0009",
          employeeName: "SURENDRA KUMAR YADAV",
          designation: "ASSISTANT MANAGER",
          number: "9415290245",
          email: "surendrarahulyadav1989@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0015",
          employeeName: "MANOJ KUMAR SHARMA",
          designation: "SENIOR MANAGER",
          number: "9560888001",
          email: "manojsharma6533@yahoo.in",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0018",
          employeeName: "NEERAJ SINGH",
          designation: "SENIOR EXECUTIVE",
          number: "9891804052",
          email: "bishtneeraj970@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0021",
          employeeName: "KUMAR MALAY",
          designation: "SENIOR PROJECT ENGINEER",
          number: "8757216984",
          email: "kumarmalaysrivastav@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0022",
          employeeName: "SARATH KUMAR PANDIYAN",
          designation: "DEPUTY PROJECT MANAGER",
          number: "8929443149",
          email: "sarath.dce@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0030",
          employeeName: "ASHOK SINGH",
          designation: "SENIOR FOREMAN",
          number: "6397538030",
          email: "ashokkumarji879@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0043",
          employeeName: "VIRENDAR SINGH",
          designation: "FOREMAN",
          number: "9718020662",
          email: "virendarsingh7446@gmai.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0044",
          employeeName: "GANESH KUMAR CHOUDHARY",
          designation: "ASSISTANT FOREMAN",
          number: "9990820954",
          email: "ganeshchy5@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0047",
          employeeName: "ARUN KUMAR",
          designation: "JUNIOR ENGINEER",
          number: "9355751220",
          email: "arun.kumarbsacet@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0067",
          employeeName: "VIJAY KUMAR SINGH",
          designation: "FOREMAN",
          number: "9891970977",
          email: "vijaysinghchendel@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0080",
          employeeName: "PINKU SHARMA",
          designation: "STORE KEEPER",
          number: "9760585453",
          email: "sharmapk0085@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0095",
          employeeName: "HARNAM SINGH",
          designation: "FOREMAN",
          number: "8376800268",
          email: "harnamsisodya.2014@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0103",
          employeeName: "NARESH KUMAR",
          designation: "SENIOR FOREMAN",
          number: "9899318292",
          email: "kumarnaresh41285@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0112",
          employeeName: "MOHSIN KHAN",
          designation: "SENIOR MANAGER",
          number: "9718004591",
          email: "khanmohsin563@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0119",
          employeeName: "BIJAY KUMAR",
          designation: "FOREMAN",
          number: "8826321036",
          email: "bijay.mcpl@yahoo.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0125",
          employeeName: "RAHUL KUMAR JHA",
          designation: "PUMP OPERATOR",
          number: "9540232874",
          email: "rahulkumar9540232874@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0129",
          employeeName: "PIYUSH KUMAR SRIVASTAVA",
          designation: "SUPERVISOR",
          number: "9716721907",
          email: "krish10aug@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0145",
          employeeName: "RAKESH KUMAR THAKUR",
          designation: "STORE KEEPER",
          number: "7814096583",
          email: "rakeshkthakur1974@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0146",
          employeeName: "PRABHASH CHANDRA KUMAR",
          designation: "FOREMAN",
          number: "9999147021",
          email: "pckumar405@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0150",
          employeeName: "PORAS",
          designation: "PROJECT ENGINEER",
          number: "9416239924",
          email: "sardana.poras98@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0177",
          employeeName: "NEERAJ SHARMA",
          designation: "ASSISTANT FOREMAN",
          number: "9717436142",
          email: "neerajsharma41187@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0178",
          employeeName: "KAPIL KUMAR",
          designation: "SENIOR PROJECT ENGINEER",
          number: "9718431141",
          email: "kapilkmr.kumar0@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0191",
          employeeName: "ANIL PANDEY",
          designation: "SENIOR FOREMAN",
          number: "7011470775",
          email: "anilpandey4537@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0198",
          employeeName: "SANTOSH KUMAR SINGH",
          designation: "SUPERVISER",
          number: "9661027238",
          email: "sksingh96610@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0204",
          employeeName: "AJAY SINGH",
          designation: "FOREMAN",
          number: "8076371034",
          email: "ajaychauhan8076@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0223",
          employeeName: "ANKIT CHOHAN",
          designation: "ASSISTANT ENGINEER",
          number: "8700822568",
          email: "ankit64chauhan@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0234",
          employeeName: "UMESH PRASAD",
          designation: "FOREMAN",
          number: "9560825136",
          email: " Up675921prasad@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0244",
          employeeName: "RAM GOPAL",
          designation: "ASSISTANT FOREMAN",
          number: "9560539982",
          email: "rangers72423@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0245",
          employeeName: "MOHAMMAD HILAL",
          designation: "PROJECT ENGINEER",
          number: "9717612501",
          email: "mohdhilal61@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0254",
          employeeName: "SANDEEP KUMAR MISHRA",
          designation: "ASSISTANT FOREMAN",
          number: "8750198311",
          email: "sandeepkmishra@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0260",
          employeeName: "YASHPAL SINGH",
          designation: "ASSISTANT ENGINEER",
          number: "9758432778",
          email: " Ys233288@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0262",
          employeeName: "GULAB CHANDRA PAL",
          designation: "ASSISTANT FOREMAN",
          number: "9871863363",
          email: " Gulabpal127@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0265",
          employeeName: "ASHRAF ALI",
          designation: "FOREMAN",
          number: "9821732535",
          email: "ashrafalip4@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0273",
          employeeName: "RAVI RANJAN KUMAR",
          designation: "FOREMAN",
          number: "8750403996",
          email: "raviranjanku008@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0278",
          employeeName: "ARVIND KUMAR",
          designation: "FOREMAN",
          number: "9891864355",
          email: "kmra620@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0290",
          employeeName: "SANJIT KUMAR RAY",
          designation: "FOREMAN",
          number: "8373997505",
          email: "sanjeet.ray201090@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0292",
          employeeName: "UMASHANKAR",
          designation: "LAB TECHNICIAN",
          number: "9453901021",
          email: "us0461817@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0304",
          employeeName: "SHRI SHYAM YADAV",
          designation: "PROJECT ENGINEER",
          number: "9654075419",
          email: "srishyamyadav1991@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0310",
          employeeName: "KAUSHAL CHOUHAN",
          designation: "FOREMAN",
          number: "9910570143",
          email: "kaushal.unnati80@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0317",
          employeeName: "BALAM SINGH RAWAT",
          designation: "MEDICAL ASSISTANT",
          number: "9971923673",
          email: "ssrawat07031969@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0324",
          employeeName: "RAM SAKAL SHARMA",
          designation: "FOREMAN",
          number: "8448707163",
          email: "ramsakal9amrapali@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0327",
          employeeName: "BEERESH",
          designation: "FOREMAN",
          number: "9582487994",
          email: "beereshkumar503@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0328",
          employeeName: "ALOK KUMAR TIWARI",
          designation: "SENIOR EXECUTIVE",
          number: "9582595587",
          email: "aloktiwari187@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0341",
          employeeName: "RANA SINGH",
          designation: "ASSISTANT ENGINEER",
          number: "8279713039",
          email: "singhrana233@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0364",
          employeeName: "PARDEEP KUMAR",
          designation: "FOREMAN",
          number: "7456883587",
          email: "pradeepbazad4433@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0368",
          employeeName: "MANISH SINGH",
          designation: "FOREMAN",
          number: "9990170977",
          email: " LEFT",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0373",
          employeeName: "ASHOK SINGH",
          designation: "FOREMAN",
          number: "9268572027",
          email: "singhashok0000@gmil.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0428",
          employeeName: "DHANANJAY SINGH",
          designation: "FOREMAN",
          number: "9278925793",
          email: "dhananjay.ds833@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0440",
          employeeName: "CHAITNAY ATTRI",
          designation: "PROJECT ENGINEER",
          number: "9084717878",
          email: "chaitnayattri@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0474",
          employeeName: "RAJESH SHARMA",
          designation: "TIME KEEPER",
          number: "7906755563",
          email: "rrajeshsharma0176@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0488",
          employeeName: "SIDHANTH SINGH",
          designation: "JUNIOR ENGINEER",
          number: "6393159800",
          email: "singhsiddhant024@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0490",
          employeeName: "AWADHESH KUMAR",
          designation: "SENIOR-STORE KEEPER",
          number: "8585981432",
          email: "yadavawadhesh01@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0491",
          employeeName: "CHANDER DEV YADAV",
          designation: "SENIOR LAB TECHNICIAN",
          number: "9304220896",
          email: "chanderdevyadav655@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0496",
          employeeName: "RAHUL KUMAR",
          designation: "SENIOR EXECUTIVE",
          number: "8130647892",
          email: "rahulbhatilavish@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0502",
          employeeName: "SANJIV KUMAR",
          designation: "ASSISTANT PROJECT MANAGER",
          number: "9643302401",
          email: "vermasanj1985@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0503",
          employeeName: "SANJAY SHARMA",
          designation: "SENIOR FOREMAN",
          number: "8851214729",
          email: "sanjaysharmacivil84@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0504",
          employeeName: "SUDHIR KUMAR SINGH",
          designation: "PROJECT MANAGER",
          number: "8377002834",
          email: "sk_10779@yahoo.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0505",
          employeeName: "AMIT KUMAR MISHRA",
          designation: "SENIOR SUPERVISOR",
          number: "8874491722",
          email: "amitkumarmishra@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0530",
          employeeName: "NARENDRA SHARMA",
          designation: "FOREMAN",
          number: "9871312449",
          email: "narendrasharma0031@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0533",
          employeeName: "ANOOP KUMAR",
          designation: "ASSISTANT GENERAL MANAGER",
          number: "7291999591",
          email: "anoop.b.bhardwaj@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0536",
          employeeName: "TARUN CHAUDHARY",
          designation: "SENIOR MANAGER",
          number: "9818416836",
          email: "tarun20002k@gmail.com",
          location: "PIOUS HIDEAWAYS SALES",
        },
        {
          empID: "HK0547",
          employeeName: "ANSHUMAN SINGH",
          designation: "JUNIOR ENGINEER",
          number: "7607763463",
          email: "anshusinghrajput2018@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0555",
          employeeName: "PAWAN KUMAR MAHTO",
          designation: "FOREMAN (LIFT)",
          number: "9999597449",
          email: "pawanmahtoats29@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0557",
          employeeName: "RAJESH TYAGI",
          designation: "PROJECT ENGINEER",
          number: "9799104228",
          email: "rajeshtyagi9799104882@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0561",
          employeeName: "MOHD ADIL KAREEM",
          designation: "PROJECT ENGINEER",
          number: "8505970456",
          email: "adilkareem1993@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0563",
          employeeName: "PANKAJ KUMAR YADAV",
          designation: "SENIOR FOREMAN",
          number: "8744924906",
          email: "pankajyadav15288@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
        {
          empID: "HK0564",
          employeeName: "SINGESHWAR PANDIT",
          designation: "FOREMAN",
          number: "9718773507",
          email: "panditsingeshwar1979@gmail.com",
          location: "PIOUS HIDEAWAYS",
        },
      ],
      skipDuplicates: true,
    });

    return res.status(201).json({
      message: `${result.count} users have been successfully registered.`,
    });
  } catch (error) {
    console.error("Bulk signup error:", error);
    return res
      .status(500)
      .json({ message: "An error occurred during bulk user registration." });
  }
};

export const userSignup = async (req: Request, res: Response) => {
  const { empID, employeeName, designation, number, email, location } =
    req.body;

  // Input validation (you can expand this section as per your validation logic)
  if (!empID || !email) {
    return res
      .status(400)
      .json({ message: "Employee ID and email are required." });
  }

  try {
    // Check if the user already exists
    const existingUser = await prisma.user.findUnique({
      where: { empID },
    });

    if (existingUser) {
      return res
        .status(400)
        .json({ message: "User already registered with this Employee ID." });
    }

    // Create a new user
    const newUser = await prisma.user.create({
      data: {
        empID,
        employeeName,
        designation,
        number,
        email,
        location,
      },
    });

    return res.status(201).json({
      message: "User successfully registered.",
      user: {
        id: newUser.id,
        empID: newUser.empID,
        employeeName: newUser.employeeName,
        designation: newUser.designation,
        number: newUser.number,
        email: newUser.email,
        location: newUser.location,
      },
    });
  } catch (error) {
    console.error("Signup error:", error);
    return res
      .status(500)
      .json({ message: "An error occurred during user registration." });
  }
};

export const sendGeneratePassMail = async (req: Request, res: Response) => {
  try {
    const { empID } = req.body;

    // Validate input
    if (!empID) {
      return res.status(400).json({ message: "Employee ID is required." });
    }

    // Check if user exists
    const user = await prisma.user.findUnique({
      where: { empID },
    });
    if (!user) {
      return res.status(404).json({ message: "Employee not found." });
    }

    // Generate token
    const token = generateToken({
      id: user.id,
      empID: user.empID,
    });

    // Send email with token
    await sendGeneratePasswordMail({
      // @ts-ignore
      email: user.email,
      token,
    });

    return res
      .status(200)
      .json({ message: "Password reset email sent to your email." });
  } catch (error) {
    console.error("Error sending password reset email:", error);
    return res.status(500).json({
      message: "An error occurred while sending password reset email.",
    });
  }
};

export const userGeneratePassword = async (req: Request, res: Response) => {
  try {
    const { password } = req.body;
    const { token } = req.params;

    if (!token) {
      return res.status(400).json({ message: "Token is required." });
    }

    // Verify token
    let verifyToken;
    try {
      // @ts-ignore
      verifyToken = jwt.verify(token, process.env.JWT_SECRET);
    } catch (error) {
      console.error("Error verifying token:", error);
      return res.status(400).json({ message: "Invalid token." });
    }

    // Check if token verification failed
    if (!verifyToken) {
      return res.status(400).json({ message: "Invalid token." });
    }
    const hashedPassword = await bcrypt.hash(password, saltRounds);
    const generatePassword = await prisma.user.update({
      where: {
        // @ts-ignore
        empID: verifyToken.empID,
      },
      data: {
        password: hashedPassword,
        isVerified: true,
      },
    });

    return res
      .status(200)
      .json({ data: generatePassword, message: "Password Generated!" });
  } catch (error) {
    console.error("Error generating password:", error);
    return res
      .status(500)
      .json({ message: "An error occurred while generating the password." });
  }
};

export const userLogin = async (req: Request, res: Response) => {
  try {
    const { empID, password } = req.body;

    // Validate input
    if (!empID || !password) {
      return res
        .status(400)
        .json({ message: "Employee ID and password are required." });
    }

    // Find user by empID
    const user = await prisma.user.findUnique({
      where: { empID },
    });

    // Check if user exists
    if (!user) {
      return res.status(404).json({ message: "Invalid credentials." });
    }

    // Compare passwords
    //   @ts-ignore
    const matchPassword = await bcrypt.compare(password, user.password);
    if (!matchPassword) {
      return res.status(401).json({ message: "Invalid credentials." });
    }

    const accessToken = generateAccessToken({
      id: user.id,
      empID: user.empID,
    });
    const options = {
      sameSite: false,
      secure: false,
    };
    res.cookie(`Homekraft`, accessToken, options);

    // Password is correct, return user data
    return res.status(200).json({ data: user, message: "Login successful." });
  } catch (error) {
    console.error("Login error:", error);
    return res.status(500).json({ message: "An error occurred during login." });
  }
};

export const userProfile = async (req: Request, res: Response) => {
  try {
    const decodedToken = req.userToken;
    const user = await prisma.user.findUnique({
      where: { id: decodedToken.id },
      select: {
        id: true,
        empID: true,
        employeeName: true,
        designation: true,
        number: true,
        email: true,
        location: true,
        isVerified: true,
        isAdmin: true,
        createdAt: true,
      },
    });

    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }

    return res.status(200).json({ data: user, message: "User fetched" });
  } catch (error) {
    console.error("Error fetching user:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
};

export const sendForgetMail = async (req: Request, res: Response) => {};

export const userForgetPassword = async (req: Request, res: Response) => {};

export const userUpdatePassword = async (req: Request, res: Response) => {
  try {
    const decodedToken = req.userToken;

    const { oldPassword, newPassword } = req.body;
    const user = await prisma.user.findUnique({
      where: { id: decodedToken.id },
    });
    // If user doesn't exist
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }

    // Verify old password
    // @ts-ignore
    const isPasswordValid = await bcrypt.compare(oldPassword, user.password);
    if (!isPasswordValid) {
      return res.status(400).json({ error: "Old password is incorrect" });
    }

    // Hash the new password
    const hashedNewPassword = await bcrypt.hash(newPassword, saltRounds);

    // Update admin's password
    await prisma.user.update({
      where: { id: decodedToken.id },
      data: { password: hashedNewPassword },
    });

    // Clear the "campus" cookie
    res.clearCookie("Homekraft");
    return res.status(200).json({ message: "Password changed successfully" });
  } catch (error) {
    console.error("Error changing in password:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
};

export const userLogout = async (req: Request, res: Response) => {
  try {
    // Clear the "campus" cookie
    res.clearCookie("Homekraft");
    return res.status(200).json({ message: "Logout successful" });
  } catch (error) {
    console.error("Error logging out:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
};
